...
| `msl_community_joined` | `'1'` | `HomeTab` (WhatsApp community card — Join button or "Already in" button) | Flag indicating user joined the WhatsApp group OR self-declared "already in." Once set, the community card no longer renders on Home. No automatic verification of actual group membership. Added v4.113. |
| `msl_cert_issued_at` | `string (Unix ms timestamp)` | `ProfilePage` (Card 2) on first render when MLE Path is complete (`foundationsComplete === true`) | Stamps the moment MLE Path completion was first acknowledged in ProfilePage. Used to render the certificate's "completed on" date + certificate ID. Once set, stable across visits — gives the certificate a stable identifier. Added v4.113. |

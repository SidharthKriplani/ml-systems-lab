...
#### Interview Trainer Prompt — free resource card on PlansTab (Source: product discussion, 2026-06-06)
- [x] ~~**Interview Trainer Prompt freebie on PlansTab**~~ — ✅ Done v4.76. `TRAINER_PROMPT` constant in PlansTab.jsx. "Free resource" card below the feature table: description + copy-to-clipboard button. Full interview prep control system prompt (versioned trace, timed drills, anti-memorization, scoring, progress dashboard). Users paste into Claude/ChatGPT + resume + JD — no LLM in MSL. Architecture decision in DECISIONS.md.
- [ ] **"Start Interview Sim" context export (follow-on)** — MSL reads localStorage (weak modules, scores, session memory) and generates a pre-filled context block the user pastes alongside the trainer prompt. Trainer is immediately context-aware without manual setup. Pure string template, no API calls. S effort.

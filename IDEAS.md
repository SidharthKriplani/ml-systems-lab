...
#### Interview Sim Context Export — "Start Sim" button (Source: product discussion, 2026-06-06)
- [ ] **"Start Interview Sim" export button in Interview zone** — MSL already tracks weak modules, session scores, and practice history via localStorage. A button (likely on HomeTab or InterviewGrid) generates a pre-filled, copy-pasteable prompt block that includes: (1) user's weakest domains from MSL scores, (2) relevant case types from their active practice zone, (3) a pre-built session context (role, round, focus areas inferred from activity). User pastes this block into Claude or ChatGPT alongside the Interview Trainer system prompt + their resume/JD, and the trainer is immediately context-aware — no manual setup. **Architecture decision: MSL is the context generator, the LLM is the trainer. The trainer prompt does NOT live inside MSL.** No backend needed, no API calls, no LLM in MSL. Pure localStorage read + string template. S effort. Source: Avinash built a full interview trainer system prompt (versioned trace, timed drills, anti-memorization rules, progress dashboard) — this bridges that tool with MSL's practice data.
...
- [x] ~~**Emoji → SVG sweep**~~ — done (v4.46, 2026-06-02). 97 replacements across 8 tabs. New Icons.jsx component for CheckMark, CrossMark, WarningMark.
...
*Full done history is in LINEAGE.md — canonical record. Items listed here are superseded by LINEAGE entries.*

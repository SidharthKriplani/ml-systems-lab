...
MSL-specific same night: **tracks cross-device sync CONFIRMED WORKING from Sidharth's
phone** (note Created 16 Jul on the MacBook, Edited 17 Jul 00:20 on the phone) after the
root-cause fix: MSL's `user_progress.value` column is TEXT (GSL's is jsonb) — the raw
{tracks, tombstones} object was rejected by Postgres on every push until tracksSync.js
stringified on push / parsed on pull.

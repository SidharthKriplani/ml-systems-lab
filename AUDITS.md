...
- **R4 — Orphans with real content (✅ RESOLVED 2026-06-24 — archive verdict REVERSED).** Content comparison (skipped in the first pass) proved all three orphans were UNIQUE, not redundant. **DataScienceTab**: 24 scenarios, 0 live-dups; ~11 experimentation = PAL (stays archived), 13 MSL calibration/metric-pitfall scenarios **harvested into ModelEval** (new Metric Pitfalls module + Calibration 6→14). **AskTab**: 36/41 unique → **restored + merged with search**. **GlobalSearch**: unique 196-entry INDEX **borrowed into `searchIndex.js`** (powers AskTab search); shell stays archived. **Rule added: "unwired" ≠ "redundant" — compare content before archiving.**
...
| 039.6 | Practice section thinness: Incident Room 6 scenarios, ML Coding 7 problems — below 12-minimum threshold set in DECISIONS.md | IncidentRoomTab.jsx, MLCodingTab.jsx | Medium | ✅ Fixed v4.73 — Incident Room 12/12, ML Coding 12/12 |

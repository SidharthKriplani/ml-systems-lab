...
A model that randomly guesses produces a diagonal line — TPR equals FPR at every threshold, so AUC = 0.5. A perfect model hugs the upper-left corner — TPR = 1 at FPR = 0, so AUC = 1.0. For fraud detection, AUC = 0.85 is reasonable.

[FIGURE: roc_curve]

There is a second way to compute AUC without drawing the curve at all. AUC equals $P(\\text{score(positive)} > \\text{score(negative)})$ — the probability that a randomly drawn positive example is ranked above a randomly drawn negative example. This is the normalized Mann-Whitney U statistic: enumerate every pair (one fraudulent transaction, one legitimate transaction), score 1 if the fraud scores higher, 0.5 if tied, 0 otherwise, then average over all pairs. For AUC = 0.91: pick any one fraud and any one legitimate transaction at random — the model ranks the fraud above the legitimate one 91% of the time. This interpretation makes AUC's ceiling clearer: it rewards correct pairwise ranking, averaged uniformly over all possible pairs. A model that is barely distinguishable near threshold 0.5 and perfectly discriminating near threshold 0.9 gets the same AUC as one that is uniformly good at all thresholds — because both get the same pairwise score on average.
...
K-fold CV runs k different splits and averages the results. With k=5 and 5,000 examples: split into 5 groups of 1,000. Train on 4,000, evaluate on 1,000. Rotate — use a different group as the held-out set each time. Average the 5 scores. You now have a much more stable estimate of generalization performance, because each example appears in the held-out set exactly once. Standard practice: k=5 or k=10. k=5 uses 80% of data per fold. k=10 uses 90% but is slower. Leave-one-out (k=n) is theoretically appealing but has high variance across folds — a single anomalous validation example swings the score significantly.

[FIGURE: cv_folds]

Standard k-fold works only when your rows are independently drawn from the same distribution. The moment you deviate, you need a different strategy.

Stratified k-fold: use whenever class balance is not uniform. With 5% positive rate and k=5 folds, a naive random split can produce one fold with 1% positives (too easy) and another with 9% (too hard). The AUC estimates across folds are measuring different problems and averaging them is misleading. Stratified k-fold enforces that every fold contains approximately 5% positives. Always stratify for any classification task with positive-class rate below 20%.

Group k-fold: use when the same entity appears multiple times in your data, and the model will be deployed on entities it has never seen. Consider a hospital outcome model: 50,000 visits from 5,000 patients. With standard k-fold, patient 142's January visit is in the training fold and their June visit is in the validation fold. The model memorises patient 142's diagnoses and history during training, then "predicts" their June outcome using that memorised context. At deployment, the model faces entirely new patients. Group k-fold puts all of a patient's visits into one fold only. The validation fold always contains patients the model has never touched — which is the actual deployment scenario.

On time-series data, standard k-fold is wrong — not a rough approximation, actually wrong. It lets the model train on month 10 data to predict month 3 events. The correct approach is walk-forward validation: train on $[t_0, t_1]$, validate on $[t_1, t_2]$. Then expand: train on $[t_0, t_2]$, validate on $[t_2, t_3]$. Temporal order is preserved. But there is a subtlety that most implementations miss: if your features include any rolling window (7-day rolling average, 14-day lag), then a training example from day 89 and a validation example from day 91 share raw data in their feature computation — the feature for day 91 is computed using days 85–91, which overlaps with training data. Add a purge gap: discard all examples within one feature window of the train-validation boundary. If your rolling window is 7 days, purge the 7 days immediately before the start of the validation window. The training window ends day 83; the validation window begins day 91.

**NOT this.** "Cross-validation gives the true test error." CV gives an estimate of expected performance on new data from the same distribution drawn the same way. It does not protect you if the distribution shifts between training time and deployment. It cannot prevent leakage if your features were computed using future information before the split. And if you tune hyperparameters on the same validation set you evaluate on, selection bias inflates your reported numbers by an amount that grows with how many configurations you tried — running 200 Optuna trials and reporting the best trial's validation AUC is upward-biased by selection even on a proper held-out fold. The unbiased alternative is nested CV: outer loop estimates generalisation quality on data completely untouched during hyperparameter search; inner loop runs the full search. It is slower but the only approach that gives an unbiased estimate of the full model-selection pipeline.`,
    keyPoints: [

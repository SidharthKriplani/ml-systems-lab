export const PRODUCTION_MODULES = [
  {
    ...
    summary: `A model trained on batch SQL features and served with real-time Python logic will behave differently in production even if both codepaths are "correct." Different timezone handling, different null treatment, different aggregation window boundaries — the model was trained on one distribution and is evaluated on another. No alarm fires when this happens. Offline metrics look fine the whole time because they measured the training world. The serving engineer rewrites a null-handling clause. The feature value shifts for 12% of requests. The model's predictions in that region become unreliable. No exception is thrown. The only fix that actually holds is a single shared feature computation function used in both the training pipeline and the serving layer, with preprocessing parameters serialized into the model artifact. Two separately maintained codepaths will always drift — no amount of documentation or code review prevents it.`,
    keyPoints: [
      `**The most common cause is two separate implementations of the same conceptual feature.** A Python serving endpoint reimplementing what a Spark training job computed will accumulate subtle differences over months — timezone handling, null treatment, aggregation window boundaries. None of these look like bugs. They look like equivalent code. They produce systematically different feature values.`,
      `**Timestamp skew is the most insidious form.** If "account_age_days" is computed as (training_run_date - account_open_date), every training sample gets a value anchored to the training date. At serving six months later, the same account is six months older than the model expects. The model learned "older accounts are lower risk" from this artifact and now sees a systematically shifted value at inference. Fix: always compute relative to the observation event timestamp, never the pipeline run date.`,
      `**Detection requires feature logging.** Log production features alongside predictions, then run PSI and KS tests comparing the training feature distribution to the production feature log for each feature. Any feature with PSI > 0.1 goes on the investigation list. You cannot debug skew you did not log — the logging infrastructure must be built before it is needed, not after the incident that reveals skew.`,
      `**Shadow mode is the best pre-launch detection available.** Route real production traffic to the new model — it receives live features, logs everything, and its predictions never reach users. Compare the logged feature distributions against training data. Skew shows up here in a safe environment before any user is affected, rather than after deployment when the business has already absorbed the impact.`,
      `**Preprocessing skew is consistently underestimated.** The StandardScaler's mean and std are fit on training data. If the serving code refits the scaler from production data, or assumes default zero-mean unit-variance parameters, the normalized values land in a completely different range than what the model trained on. The fix: serialize the fitted scaler as part of the model artifact. It travels with the weights, is loaded at serving time, and produces identical normalized values by construction.`,
      `**Remediation is infrastructure, not a one-time debugging fix.** Wrap model and preprocessor in a single sklearn Pipeline object. Serialize the entire Pipeline as one artifact. Deploy the artifact. The scaler's fitted parameters are then identical in training and serving by construction — not by convention, not by documentation, by the artifact itself. Any other approach relies on discipline that will eventually fail.`,
      `**Point-in-time correct feature extraction: for each training sample at label timestamp t, retrieve feature values as they existed at time t — not as they exist when the training pipeline runs.** This requires a temporal join: SELECT feature_value WHERE entity_id = ? AND feature_timestamp <= event_timestamp ORDER BY feature_timestamp DESC LIMIT 1. Training on today's feature values for historical events produces a model that has never seen the feature values it will actually encounter at serving time.`,
      `**Continuous monitoring: track mean, std, null rate, and PSI per feature per deployment.** Alert at PSI > 0.2 and null rate changes of more than 2x. Null rate changes are the most common early signal — a feature moving from 0.5% null to 5% null means something upstream changed, and the model is now imputing where it used to have real data. This shift is often detectable weeks before ground-truth labels confirm performance degradation.`,
    ],
    checkQuestions: [
      {

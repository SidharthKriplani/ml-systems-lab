...
Take a 2-layer network with input x = 2.0, weights w₁ = 0.5 and w₂ = 0.3, and target y = 1.0. Forward: compute the first hidden value, squash it with a sigmoid to get h₁ = 0.75 (cache it), carry on to the prediction ŷ = 0.556, and the loss comes out to 0.197.
...
Go layer by layer, multiplying the local slopes together (that is the chain rule): the gradient for w₂ works out to about −0.164, and the gradient for w₁ to about −0.025. w₁'s gradient is already **6.6× smaller** than w₂'s, and the reason is exactly that extra handoff: reaching one layer deeper multiplied the message by the sigmoid's slope (about 0.19). One extra floor, one shrink.

---

**Why deep networks stalled: vanishing gradients.**

That shrink is the whole vanishing-gradient story, just repeated at every floor. A sigmoid's slope is *at most* 0.25, and usually less. Multiply that in at every handoff, and after 10 layers the message reaching the first layer is scaled by roughly 0.25¹⁰ — about one in a million; after 20 layers, essentially zero. The instructions sent from the top floor arrive at the bottom as silence. The early layers get no signal and never learn, so your "20-layer network" quietly behaves like a 2-layer one. The fix is **ReLU**, whose slope is a clean 1 for active neurons, so the message passes down every floor untouched. (ReLU's own catch is the *dead neuron* — one whose input is always negative outputs zero forever, dropping out of the relay; Leaky ReLU and GELU keep a trickle of gradient flowing to prevent that.)
...
All of the above is about the *hidden* layers. The *output* activation is a correctness constraint, not a preference. For a probability (binary classification) you must use **sigmoid**; for a set of class probabilities, **softmax**; for a plain number (regression), no activation at all. Putting a ReLU on the output of a classifier would let it emit nonsensical values — you match the output activation to what the answer is supposed to *be*, and that is a rule, not a tuning knob.`,
    ...
    recap: [
      `**Activation = the non-linear squash applied after each layer:** it's what lets a network bend space instead of only drawing straight lines — and it also decides whether the learning signal survives the trip backward through the layers. The whole history of activations is a sequence of gradient-flow fixes.`,
      `**Sigmoid kills gradients:** its slope is at most 0.25, multiplied in at every layer → ~0.25¹⁰ ≈ one-in-a-million after 10 layers. The early layers stop learning and a 10-layer network effectively acts like a 2-layer one — the reason deep nets were so fragile before ReLU.`,
...
Here is the twist. On image-classification benchmarks, a well-tuned **SGD with momentum** often *beats* Adam on validation accuracy, even though Adam trains faster. The reason ties back to the sharp-versus-flat-minima idea: Adam's per-weight rescaling lets it slide neatly into the *nearest* minimum, which tends to be a sharp, narrow one. Plain SGD keeps more of its gradient noise, which jostles it toward *flatter, wider* minima — and flat minima generalise better, especially when the test data drifts from training. So the trade is real: Adam gives you speed, SGD (tuned, with patience) can give a slightly better final model. Whichever you use, never leave the learning rate fixed for the whole run — decay it over time, and for Transformers *warm it up* first (start tiny and ramp up over the first few thousand steps), or the early, unreliable gradients will blow training up.`,
    ...
    recap: [
      `**One fixed learning rate can't suit every weight:** a rare-word embedding sees a gradient maybe once in 1000 steps and needs big pushes when it does; a final-layer weight sees one every step and needs small ones. A single global step size serves neither well.`,
      `**Adaptive optimisers give each weight its own effective learning rate** derived from its gradient history — big steps for parameters with small/rare gradients, small steps for those with large frequent ones.`,
      `**The lineage:** AdaGrad divides by the *sum* of past squared gradients (good for sparse features, but the sum only grows so on dense problems the rate decays to zero and training stalls) → RMSProp swaps the sum for a decaying average so it never collapses → Adam adds a momentum term on top of RMSProp. Adam is the default for most deep learning.`,
...
**NOT this.** "CNNs were designed for images." The principle — local patterns plus translation equivariance — applies anywhere locality matters. 1D CNNs classify audio and DNA sequences, where adjacent time steps or nucleotides are locally related. 3D CNNs process video, where nearby frames in time are locally correlated. Graph CNNs extend the idea to molecular structures and social networks. The architecture is not about pixels; it is about exploiting whatever spatial or sequential structure your data has. If your input has the property that neighboring elements are more related than distant elements, a convolutional inductive bias is appropriate. If your input is a bag of features with no meaningful ordering, it is not.`,
    ...
    recap: [
      ...
      `**Convolution = weight sharing:** a 3×3 filter (9 weights) slides across the whole image, computing the same dot product at every location — learn a horizontal-edge detector once and it fires on edges *everywhere*, because the same weights do the work everywhere. That's 9 params where a flat layer needs ~100K.`,
...
The catch is cost: comparing every word with every other word is **O(n²)** — double the sequence length and you quadruple the work and memory. That quadratic cost is the single biggest constraint on long-context Transformers, and a whole family of tricks (FlashAttention, sparse attention, and others) exists to tame it.`,
    ...
    recap: [
      `**Core question:** how do you let a word look at exactly the other words that matter to it (e.g. "bank" needs to see nearby "river" to disambiguate)? That single idea is attention, and it underlies every Transformer.`,
      `**Old encoder-decoder RNNs squeezed the whole sentence into one fixed-size summary vector** before decoding began — by the time it reached "bank," that summary had been overwritten word by word and the "river" clue was diluted or gone. Long sentences lost exactly the information that mattered.`,
      `**Attention fixed it directly:** instead of one squeezed summary, each word *reaches back and looks at all the other words at once*, weighting them by relevance — so translating "bank" can put most of its weight on "river" no matter how many words sit in between.`,
      `**Q, K, V — three learned views of each word:** Query ("what am I looking for?"), Key ("what do I offer?"), and Value ("what I hand over if you pick me?"). To decide how much word *i* attends to word *j*, you compare i's query with j's key.`,
      `**Mechanism:** score = Q·K (a dot product — big match = relevant), run all scores through a softmax so they become weights summing to 1, then take the weighted average of the *values*. Divide scores by √d before the softmax, or in high dimensions the dot products get so large the softmax goes near one-hot and gradients dry up.`,
      `**Multi-head attention runs several in parallel,** each with its own Q/K/V — one head can track grammar, another meaning, another position — giving the model several lenses instead of forcing everything through one. Self-attention also gives an O(1) path between any two positions, which RNNs can't match without exponential gradient attenuation.`,
      `**The catch is O(n²) cost:** comparing every word with every other word means doubling the sequence length quadruples the work and memory — the single biggest constraint on long-context Transformers, and what FlashAttention, sparse attention, and every efficient variant are trying to cut.`,
    ],
    checkQuestions: [
      {
...
Fine-tuning has a trap called **catastrophic forgetting**: hit the pre-trained model with a big learning rate on your small dataset and you *overwrite* the very knowledge that made it valuable, collapsing it onto your narrow task. The standard safety recipe is a *gentle* touch — a learning rate 10–100× smaller than pre-training used, only a few epochs, a short warmup, and a little weight decay — small enough steps that you *stay near* the pre-trained starting point instead of wandering off and erasing it.`,
    ...
    recap: [
      ...
      `**Pre-training is self-supervised:** the model does fill-in-the-blank (masked) or next-word prediction on a mountain of *unlabelled* text, which forces it to internalise grammar, facts, and structure into its weights before it ever sees your task.`,
      `**Fine-tuning nudges a short distance,** not teaching from scratch — it starts in a loss basin already most of the way to a good solution and moves the weights only a little, which is why a few hundred labelled examples suffice.`,

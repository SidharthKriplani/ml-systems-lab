export const MONITORING_MODULES = [
  ...
  },
  {
    ...
    summary: `Input features change in production constantly — new user cohorts, seasonal patterns, upstream pipeline changes, product launches. Without systematic detection, these shifts accumulate silently until model performance degrades enough to show up in business metrics. By then the root cause is buried under weeks of compounding changes. Data drift detection solves the early-warning problem: compare the current serving distribution of each feature against a reference (usually the training distribution) and quantify divergence numerically. The challenge is choosing the right statistical test for each feature type and setting thresholds that distinguish meaningful drift from sampling noise. Too sensitive and engineers spend all their time investigating harmless fluctuations. Too conservative and real drift goes undetected until damage accumulates.`,
    keyPoints: [
      `**PSI (Population Stability Index): the standard drift metric in financial services.** PSI = Σ (A_i - E_i) × ln(A_i/E_i) where A_i is the actual fraction of data in bucket i and E_i is the expected fraction from the reference. PSI below 0.1 means no significant shift. Between 0.1 and 0.2 means moderate shift worth investigating. Above 0.2 signals significant shift requiring action. PSI's bucket-based approach works for both numerical and ordinal features, and the threshold rules are stable enough that teams can apply them without re-calibrating per feature.`,
      `**PSI bin construction: use 10 equal-frequency bins derived from training data so each bin represents the same volume of historical data.** For new data falling outside the training range, add boundary bins to capture extreme values. Getting bin construction wrong — using equal-width bins on a skewed distribution — causes PSI to be dominated by the densely-populated center buckets while missing meaningful changes in the tails.`,
      `**KS test for continuous features: D = max|F_reference(x) - F_current(x)|, the maximum difference between the reference and current CDFs at any point.** Distribution-free, so no assumptions about the underlying distribution. Provides a p-value. The critical failure mode: with very large samples (1M+ daily requests), the KS test flags trivially small differences (D=0.01) as statistically significant. Statistical significance is not operational significance. Always define a minimum practical threshold (D > 0.05) independent of the p-value.`,
      `**Chi-squared test for categorical features: tests whether the observed category frequency distribution matches the expected distribution from training. χ² = Σ (O-E)²/E.** With many categories (high-cardinality features like device model or zip code), degrees of freedom are high and you need sufficient expected counts per category — fewer than 5 expected observations per bucket makes the chi-squared approximation unreliable. Collapse rare categories into an "other" bucket before applying the test.`,
      `**Jensen-Shannon divergence: JSD(P||Q) = (1/2)KL(P||M) + (1/2)KL(Q||M) where M=(P+Q)/2.** Unlike KL divergence, JSD is symmetric (JSD(P||Q) = JSD(Q||P)) and bounded between 0 and 1 with log base 2. JSD=0 means identical distributions; JSD=1 means completely non-overlapping. The square root of JSD is a proper metric, enabling consistent threshold comparison across features with different natural variability. This makes JSD easier to threshold uniformly across a feature set than KS or PSI.`,
      `**Threshold calibration: a single threshold applied to all features produces too many false positives on high-variance features and misses real drift on stable features.** Calibrate per feature: look at historical variability in the feature's distribution over periods when model performance was known to be good, and set the alert threshold above that natural variability. A feature like "user age" that's historically stable needs a lower threshold than "session duration" which varies by day of week.`,

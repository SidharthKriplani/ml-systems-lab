...
d N_a(T) is the number of times you pull it.** Minimising regret means minimising pulls on suboptimal arms — but identifying which arms are suboptimal requires the exploration you are trying to limit. This is the circular dependency that makes the problem hard.`,
  ...
  },
  {
    ...
    summary: `The simplest exploration strategy is also the most commonly deployed: with probability ε, pick a random arm; otherwise pick the empirically best arm. Epsilon-greedy's problem is that it explores uniformly — an arm with 2% CTR when the best arm has 90% CTR still gets ε/K of the exploration budget, the same as a genuinely uncertain arm. There is no mechanism to focus exploration where it matters. More fundamentally, fixed ε causes linear regret: the constant exploration rate ε·T grows without bound, so the total cost of random arms grows linearly. Annealing ε to zero is not optional — it is mathematically necessary for sub-linear regret.

In practice, epsilon-greedy survives in production because its simplicity makes it easy to deploy, debug, and audit — but its failure modes (uniform exploration, stale means, cold start) are well-known and often handled with patches.`,
    keyPoints: [
      `**Pure exploitation (ε=0) fails whenever initialisation is unlucky.** If arm A has empirical mean 0.8 from a few lucky early pulls and arm B has true mean 0.9 but was never pulled enough to correct its estimate, a greedy policy stays on A forever — linear regret O(T). The model's reward prediction for B is wrong, and pure exploitation never generates the data to correct it.`,
      `**Fixed ε causes linear regret.** In every exploration round, the expected per-step regret from random arm selection is (1/K)·Σ_a Δ_a = Δ̄. Over T rounds, exploration regret ≥ ε·T·Δ̄ — linear in T. No matter how small ε is, as long as it is positive and constant, total regret grows linearly. Annealing ε_t → 0 is required for sub-linear regret: with ε_t = c/t, total exploration regret = c·Δ̄·Σ_t 1/t = O(log T).`,
      `**Uniform exploration is the main inefficiency. ε-greedy explores all arms with equal probability ε/K, regardless of how clearly inferior they are.** An arm with empirical mean 0.1 when the best arm has mean 0.9 still receives ε/K of traffic. UCB and Thompson Sampling avoid this — they focus exploration on arms that are genuinely uncertain, not arms that are clearly inferior.`,
      `**Annealing schedules: 1/t annealing (ε_t = ε_0/t) is aggressive and underexplores if T is large. 1/√t gives more exploration early.** Explore-then-commit (ε-first) explores uniformly for m rounds then commits to the empirical best — optimal m = T^{2/3}(K ln K)^{1/3} gives O(T^{2/3}) regret. Worse than UCB's O(log T) but useful when exploration and exploitation must be temporally separated.`,
      `**High variance early is a real problem.** With K=100 arms and T=1000 total initial pulls, each arm gets ~10 pulls. Standard error for Bernoulli(0.5) rewards is 0.5/√10 ≈ 0.16. The exploit step during warmup selects based on noise, not signal — the empirical winner after 10 pulls per arm may not be the true best arm.`,
      `**Non-stationary rewards break empirical means.** CTR changes due to creative fatigue, news events, or seasonal patterns mean yesterday's mean is not today's mean. An arm with 3 months of 8% CTR has mean estimated from 90 days of data — a single day at 2% barely moves the estimate. Solutions: sliding window (only last W observations), discounted mean (μ̂_a ← γ·μ̂_a + (1−γ)·r), or periodic reset. None of these are as principled as Thompson Sampling with a proper prior.`,
      `**Cold start is the other production failure mode.** New arms start with zero observations and empirical mean 0 — they appear in the exploit step (ε=0 case) only if 0 is the highest estimate. With ε-greedy they get ε/K of traffic, which may be far too little for rapid evaluation. A new arm with K=50 and ε=0.1 gets 0.2% of traffic — at 10,000 daily impressions, that's 20 impressions per day to evaluate a new creative.`,
      `**ε-greedy survives in production because: implementation is trivial with no confidence interval computation; the low compute cost scales to billions of arms with hash-based arm selection; when arm gaps Δ are small, uniform exploration accidentally allocates reasonable budgets; and constant exploration provides automatic adaptation to non-stationary rewards.** Its deficiencies are understood and patched rather than eliminated.`,
      `**"Smaller ε is always better" is wrong.** If T is short or K is large relative to gap sizes, insufficient exploration means the best arm is never identified. The optimal ε depends on K, T, and the unknown gap structure — which is why ε-greedy requires tuning while UCB and Thompson Sampling adapt automatically.`,
    ],
    checkQuestions: [
      {
...
instance-optimal (low regret when gaps are large) but not minimax-optimal. Use UCB1 when you have reason to believe gaps are large; use MOSS when you need worst-case robustness.`,
  ...
  },
  {
    ...
    summary: `Epsilon-greedy exploration picks a random arm with fixed probability ε — it doesn't account for uncertainty. UCB tracks upper confidence bounds and explores arms where uncertainty is high. Thompson Sampling takes a different approach: maintain a probability distribution over each arm's reward probability, sample one value from each distribution, pick the arm with the highest sample. Arms with high uncertainty have wide distributions and get sampled high occasionally — exploration is proportional to uncertainty. As more data comes in, distributions narrow and the algorithm commits to the best arm. TS matches UCB's Lai-Robbins optimal O(log T) regret but typically outperforms UCB at finite horizons because posterior sampling focuses exploration proportional to P(arm a is optimal) — clearly inferior arms receive negligible exploration budget as their posteriors concentrate around low values.`,
    keyPoints: [
      `**Beta-Binomial conjugate for Bernoulli rewards: maintain Beta(α_a, β_a) per arm.** Update: on reward r ∈ {0,1} from arm a, α_a += r, β_a += (1−r). Initial uninformative prior Beta(1,1) = Uniform[0,1]. At each round: sample θ_a ~ Beta(α_a, β_a) for each arm, pull argmax_a θ_a. Arms with few observations have wide Beta posteriors — their samples span a wide range and occasionally come out high, generating exploration. Arms with many observations have concentrated posteriors.`,
      `**Gaussian Thompson Sampling: for real-valued rewards r_a ~ N(μ_a, σ²) with known σ², the conjugate prior is N(μ_0, τ_0²).** Posterior after N_a pulls: μ_{a,post} ~ N((μ_0/τ_0² + N_a·x̄_a/σ²) / (1/τ_0² + N_a/σ²), 1/(1/τ_0² + N_a/σ²)). As N_a grows, the posterior mean moves toward the empirical mean and the variance shrinks — exactly the right behaviour.`,
      `**TS exploration is proportional to P(arm a is optimal).** An arm gets pulled with probability equal to the probability that its sampled value exceeds all other arms' samples — P(θ_a = max_j θ_j | observations). As arm a accumulates observations and its posterior concentrates near a low value, this probability collapses toward zero. UCB in contrast applies a fixed confidence bonus √(2 ln t / N_a) regardless of how implausible it is that the arm could be optimal — TS gives up on clearly inferior arms faster.`,
      `**Why TS outperforms UCB at finite horizons: UCB's confidence bonus is derived from worst-case Hoeffding bounds, which are conservative.** TS uses the full posterior distribution over rewards, incorporating whatever prior information is available. Early in the experiment, a calibrated prior provides meaningful regularisation. UCB's confidence bonus is blind to prior information. The posterior shape adapts to the data in a way that a fixed formula cannot.`,
      `**Top-2 Thompson Sampling (TTTS) is for pure exploration — best arm identification, not regret minimisation.** Each round: sample all posteriors; with probability β, pull the arm with the highest sample; with probability 1−β, resample until a different arm wins, then pull that arm. TTTS ensures the second-best candidate gets exploration budget, matching the optimal fixed-budget algorithm for best arm identification.`,
      `**A/B/n tests reframed as TS: classical A/B testing uses fixed allocation throughout and applies a frequentist test at the end.** TS continuously updates posteriors and allocates more traffic to better-performing variants. The risk: standard frequentist stopping rules (p-values at fixed sample sizes) applied to TS data produce inflated Type-I error because the allocation is no longer random. Must use Bayesian stopping criteria (P(variant B is best) > 0.95) or sequential hypothesis testing (always-valid p-values) to avoid this.`,
      `**Regret analysis: TS achieves E[R_T] = O(√(KT ln K)) in the frequentist worst case and matches the Lai-Robbins lower bound asymptotically for specific parametric families (Bernoulli).** The analysis is harder than UCB because TS is randomised — the pull probability depends on the full posterior, not a deterministic formula.`,
      `**Prior misspecification is the main production failure mode.** A Beta(1,1) prior on an arm with true CTR 0.001 puts 50% of its probability mass above 0.5 — the algorithm over-explores this arm massively in early rounds because the posterior thinks it could easily be the best arm. An informative prior Beta(100, 9900) for 1% CTR solves this but introduces the opposite problem: new arms need many pulls to move the posterior significantly from the prior.`,
    ],
    checkQuestions: [
      {

export const GRAPH_ML_MODULES = [
  {
    ...
    summary: `Standard neural networks assume inputs have a fixed, ordered structure — pixels in a grid, tokens in a sequence. Graphs violate both assumptions: the number of nodes varies, and the same graph can be represented by N! different adjacency matrices depending on how nodes are numbered. Any valid graph neural network must be invariant to that reordering. Beyond the theoretical constraint, the data structure choice has immediate engineering consequences — a 50M-node social graph stored as a dense adjacency matrix requires 312 TB of memory; stored as a CSR sparse matrix it requires ~40 GB. Getting the data structure right is not a detail; the entire training and serving infrastructure depends on it.`,
    keyPoints: [
  ...
  },
  {
    ...
    summary: `Before spatial GNNs, convolution on graphs had no principled definition. Spectral methods provided one by grounding it in signal processing: the graph Laplacian acts as the frequency operator, and spectral filtering multiplies signal components in eigenspace. The problem is that eigendecomposition of the Laplacian costs O(N³) — infeasible for any real graph. ChebNet avoids it by approximating spectral filters with Chebyshev polynomials, reducing convolution to sparse matrix multiplications. The Kipf & Welling GCN simplifies further to a first-order approximation, which is what made spectral GCNs practically usable.

But the transductive limitation is fundamental and cannot be tuned away: spectral filters are defined in the eigenspace of a specific graph's Laplacian, and eigenvectors differ between graphs.
...
eter filter to a single parameter per feature. Adding self-loops (Ã = A + I) before normalization gives the propagation rule: H^{(l+1)} = σ(D̃^{-1/2} Ã D̃^{-1/2} H^{(l)} W^{(l)}). A single sparse matrix multiply per layer — tractable on large graphs. The simplification was what made GCNs widely adopted.`,
  ...
  },
  {
    ...
    summary: `Spectral GCNs learn embeddings for a specific fixed graph — add a new node and the model cannot embed it without retraining. Matrix factorization methods have the same problem. In production, new users and items arrive every second; a method requiring full graph retraining to embed new nodes is not deployable. GraphSAGE reframes graph convolution as function learning rather than matrix factorization: instead of learning node embeddings directly, it learns an aggregation function that maps any neighborhood to an embedding. The same learned function applies to nodes never seen during training. This inductivity, combined with fixed-size neighborhood sampling that bounds the exponential fan-out of multi-hop neighborhoods, is the architectural foundation of every production-scale GNN system built since 2017.`,
    keyPoints: [
  ...
  },
  {
    ...
    summary: `Mean aggregation treats all neighbors as equally informative — a reasonable prior for homophilic graphs but actively harmful when neighbors vary in relevance. A fraudster connected to 1,000 legitimate accounts and 3 fraudulent ones will have its fraud signal diluted to near-zero by mean aggregation. Graph Attention Networks replace fixed aggregation weights with learned, data-dependent attention coefficients computed from the features of both endpoints. The original GAT has a subtle flaw that matters in practice: its attention is static — the ranking of neighbors is identical regardless of which source node is querying. GATv2 (Brody et al., 2022) fixes this by placing the nonlinearity inside the concatenation, producing dynamic attention where different source nodes can rank their neighbors differently. This distinction is consequential in heterophilic and heterogeneous graphs where neighbor relevance genuinely depends on the querying node's context.`,
    keyPoints: [
  ...
  },
  {
    ...
    summary: `GCN, GAT, GraphSAGE, and most GNN variants look different architecturally, but they are all instances of the same pattern: compute messages from edges, aggregate at nodes, update node states. That unification, formalized as the MPNN framework (Gilmer et al., 2017), also reveals a hard ceiling on what all MPNNs can do: none is more powerful than the 1-dimensional Weisfeiler-Leman graph isomorphism test. Mean-aggregation GNNs like GCN and GraphSAGE are strictly below even that ceiling — mean cannot distinguish multisets with the same average, so a node with neighbors {1,2,3} looks identical to a node with neighbors {1,1,4}. GIN with sum aggregation reaches the 1-WL ceiling. The practical question is when this ceiling actually matters: for molecular chemistry and combinatorial tasks, it does. For node classification on real-world social networks, it usually doesn't.`,
    keyPoints: [
  ...
  },
  {
    ...
    summary: `Link prediction underlies friend recommendations, knowledge graph completion, and drug-target interaction prediction — commercially important and methodologically treacherous. Two traps define the field. First, structural heuristics like common neighbors often match or beat learned GNN models on homophilic networks, because triangle closure is the dominant link formation mechanism and heuristics capture it directly without training. Second, evaluation methodology is easy to get wrong: naive random edge splitting leaks structural information through shared neighbors — the GNN learns to predict test edges not because it generalizes, but because it saw paths through those edges during training. The correct procedure removes test edges from the training graph entirely before any GNN training.`,
    keyPoints: [
  ...
  },
  {
    ...
    summary: `A 2-layer GNN on a node with an average degree of 100 requires that node's 1-hop neighborhood (100 nodes) and 2-hop neighborhood (up to 10,000 nodes). A 3-layer GNN requires up to 1M nodes. Full-batch training on a graph with 100M nodes is simply impossible — this is the neighbor explosion problem, and it appears immediately when scaling beyond academic benchmarks. The production solutions — Cluster-GCN, GraphSAINT, SIGN — take fundamentally different approaches: partition the graph into mini-batches, sample random representative subgraphs, or precompute multi-hop aggregations offline. Each trades approximation quality, training stability, and engineering complexity differently. Often the accuracy gap from training-time distribution shift relative to full-graph inference is a larger practical problem than the choice of architecture.`,
    keyPoints: [
...
cale.`,
  ...
  },
  {
    ...
    summary: `Moving a GNN from an academic benchmark to a production system exposes a set of problems that benchmark papers don't mention: graphs with billions of edges, millisecond latency requirements, continuous graph updates that invalidate cached embeddings, and predictions that must be explainable to analysts and auditors. PinSage (Ying et al., 2018) is the canonical case study for this transition — from a 2-layer GraphSAGE on a toy graph to a system serving 2 billion users. Its most important innovations are not architectural: random walk importance sampling, MapReduce offline embedding computation, and ANN serving are the engineering decisions that made billion-scale GNN recsys feasible. The GNN model itself is often the simplest component of a production GNN system; the data infrastructure is not.`,
    keyPoints: [

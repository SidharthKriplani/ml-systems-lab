export const CLASSICAL_ML_MODULES = [
  {
    ...
    summary: `Linear regression models the expected output as a linear function of inputs: ŷ = Xθ. The Ordinary Least Squares (OLS) estimator minimises the sum of squared residuals ‖y−Xθ‖², and the closed-form solution θ̂ = (XᵀX)⁻¹Xᵀy is the normal equation — derived by setting the gradient to zero. Geometrically, ŷ = Xθ̂ is the orthogonal projection of y onto the column space of X, and the residuals are orthogonal to that column space. The Gauss-Markov theorem guarantees OLS is BLUE (Best Linear Unbiased Estimator) under modest assumptions — not normality, just zero-mean uncorrelated errors with constant variance. Multicollinearity makes XᵀX near-singular, causing coefficient estimates to explode; Ridge regression is the fix. R² measures the fraction of variance explained by the model, but adding any feature increases R² — use adjusted R² or holdout validation instead. Violating homoscedasticity (constant error variance) does not bias the coefficients but makes standard errors wrong.`,
    keyPoints: [
      `Normal equation θ̂ = (XᵀX)⁻¹Xᵀy: derived by setting ∂‖y−Xθ‖²/∂θ = −2Xᵀ(y−Xθ) = 0 — computationally O(nd² + d³), which means it is infeasible when d is large (d > 10,000) and iterative methods (gradient descent, conjugate gradient) are needed instead.`,
      `Geometric interpretation: ŷ = Xθ̂ is the orthogonal projection of y onto col(X) — residuals satisfy Xᵀe = 0 and are perpendicular to every column of X, which means the residuals are orthogonal to all predictions by construction, not by assumption.`,
      `Gauss-Markov theorem: OLS is BLUE among all linear unbiased estimators if errors are zero-mean, uncorrelated, and homoscedastic — normality is NOT required, which means confidence intervals via CLT hold for large n regardless of error distribution.`,
      `Multicollinearity: nearly linearly dependent features make XᵀX near-singular — small data changes cause huge coefficient swings (high variance), which means the model's predictions may be fine but the individual coefficients are meaningless and unstable.`,
      `R² = 1 − SS_res/SS_tot ∈ [0,1]: adding any feature (even noise) increases R², which means you must use adjusted R² (penalises extra features) or cross-validation to guard against spurious feature inclusion.`,
      `Heteroscedasticity: non-constant error variance — OLS is still unbiased but no longer BLUE; standard errors are wrong (t-tests and CIs are invalid), which means you should use White's heteroscedasticity-robust standard errors or switch to WLS.`,
      `OLS is the MLE for Gaussian errors: minimising ‖y−Xθ‖² = maximising N(y; Xθ, σ²I), which means the choice of squared loss implicitly assumes Gaussian noise — if errors are better modelled as Laplace, use L1 (median regression) instead.`,
      `Residual analysis: plot residuals vs. fitted values to check linearity and homoscedasticity; QQ-plot for normality; residuals vs. each feature to detect omitted non-linearities, which means residual plots are the first diagnostic before trusting any OLS inference.`,
    ],
    checkQuestions: [
      {
  ...
  },
  {
    ...
    summary: `Logistic regression models P(y=1|x) = σ(wᵀx + b) where σ(z) = 1/(1+e^{-z}). Despite the name, it is a classification model — the "regression" refers to modelling the log-odds as a linear function. The log-odds (logit) is linear in x: log[P(y=1)/P(y=0)] = wᵀx + b, which means the model is linear in the feature space and produces a hyperplane decision boundary. Training minimises cross-entropy loss, equivalent to MLE under the Bernoulli likelihood — the gradient has the clean form (ŷ − y), identical in structure to linear regression gradient. Logistic regression is well-calibrated by construction: the sigmoid maps logits to probabilities that correspond to empirical frequencies when the model is correct. When classes are perfectly linearly separable, the MLE does not exist and weights diverge to infinity — L2 regularisation is the fix.`,
    keyPoints: [
      `Sigmoid σ(z) = 1/(1+e^{-z}) ∈ (0,1); σ'(z) = σ(z)(1−σ(z)) — smooth derivative enables gradient descent, but saturates for |z| >> 0, which means the gradient ∂L/∂z vanishes when the model is very confident (correct or wrong), making learning slow near the extremes.`,
      `Log-odds interpretation: wᵀx + b = log[P(y=1)/P(y=0)] — each weight wⱼ is the log-odds ratio associated with a unit increase in xⱼ, which means coefficient interpretation is on the log-odds scale, not the probability scale.`,
      `Cross-entropy gradient wrt logit z: ∂L/∂z = ŷ − y — the prediction error, same form as linear regression, which means combining sigmoid + cross-entropy gives the cleanest possible gradient and avoids the vanishing gradient that sigmoid alone would create.`,
      `Decision boundary: P(y=1|x) = 0.5 ⟺ wᵀx + b = 0 — a hyperplane, which means logistic regression is a linear classifier and cannot learn non-linear boundaries without feature engineering or basis expansion.`,
      `Calibration: logistic regression output probabilities are well-calibrated when the model is correctly specified — P(y=1|ŷ=p) ≈ p empirically, which means predicted probability 0.7 means ~70% of those samples are positive; this is not guaranteed for tree models or SVMs.`,
      `Separability failure: when data is linearly separable, MLE drives ‖w‖→∞, which means logistic regression without regularisation will diverge — L2 regularisation (always include it) ensures a finite solution by penalising large weights.`,
      `Softmax (multinomial LR): P(y=k|x) = exp(wₖᵀx)/Σⱼ exp(wⱼᵀx) — generalises binary logistic regression to K classes, with cross-entropy loss summed over classes, which means binary logistic regression is just the K=2 special case.`,
      `L2 regularisation prevents overfitting when p >> n (features exceed samples) — equivalent to MAP with Gaussian prior, which means the regularisation strength λ should be tuned via CV and scales inversely with training set size.`,
    ],
    checkQuestions: [
      {
  ...
  },
  {
    ...
    keyPoints: [
      `Ridge (L2): θ̂_ridge = (XᵀX + λI)⁻¹Xᵀy — always invertible for any λ>0, even with perfect multicollinearity, which means Ridge is the go-to fix for ill-conditioned XᵀX and simultaneously regularises.`,
      `Ridge shrinkage: in the eigenbasis of XᵀX, Ridge scales the coefficient for component k by σₖ²/(σₖ²+λ) — small singular values (weak signal directions) are shrunk most, which means Ridge implicitly performs a soft rank truncation and filters noise in low-variance directions.`,
      `Lasso (L1): no closed form — requires coordinate descent or LARS; at the L1 ball's corners (on coordinate axes), exactly k coefficients are non-zero, which means Lasso performs automatic feature selection by driving irrelevant feature weights to exactly zero.`,
      `Geometric intuition: minimise loss s.t. ‖w‖_p ≤ r — loss contours are ellipses centred at OLS solution; the L1 diamond's corners are on the axes so the ellipse hits a corner first (sparse solution), while the L2 sphere has no corners so the solution is almost never sparse.`,
      `When L1 wins: true model is sparse (few features matter); high-dimensional data (p >> n); need interpretable, parsimonious model — the failure mode is L1 arbitrarily picks one of several correlated features.`,
      `When L2 wins: many features contribute small effects (dense signal); correlated features should be retained together; closed-form solution needed — the failure mode is L2 never exactly zeros any weight, which means you need manual thresholding for feature selection.`,
      `Elastic net: αλ‖w‖₁ + (1−α)λ‖w‖₂² — sparsity from L1 plus stability for correlated features from L2; the grouping effect means correlated features tend to get similar non-zero coefficients, which means elastic net is preferred for genomics and NLP bag-of-words.`,
      `Regularisation path: as λ increases from 0 to ∞, coefficients shrink from OLS solution to zero — Lasso path is piecewise linear (LARS algorithm), Ridge path is smooth, which means plotting the path reveals feature entry order and helps select λ.`,
    ],
    checkQuestions: [
      {
  ...
  },
  {
    ...
    keyPoints: [
      `Bias-variance decomposition: E[(y−ŷ)²] = σ² + Bias²(ŷ) + Var(ŷ) — Bias = E[ŷ]−f(x) (systematic error), Variance = E[(ŷ−E[ŷ])²] (sensitivity to training data), which means high variance = overfit (the model memorises training data) and high bias = underfit (the model misses the signal).`,
      `Regularisation controls the bias-variance tradeoff: increasing λ increases bias and decreases variance — the optimal λ balances these, which means cross-validation over λ is finding the bias-variance optimum for your dataset size.`,
      `VC dimension: the maximum number of points that a hypothesis class can shatter (label arbitrarily) — linear classifiers in ℝᵈ have VC-dim = d+1, which means every additional feature increases the hypothesis class's complexity and the risk of overfitting.`,
      `PAC learning bound: with probability 1−δ, test error ≤ train error + √(VC-dim·log(n) + log(1/δ))/n — more expressive model needs more data to generalise, which means the required sample size grows roughly linearly with VC dimension.`,
      `Inductive bias: assumptions baked into the model architecture that guide generalisation — CNNs assume translation invariance; Transformers assume permutation equivariance; RNNs assume temporal locality, which means matching inductive bias to the data structure is more important than just minimising training loss.`,
      `Double descent: test error decreases, then rises (classical overfitting), then decreases AGAIN for very overparameterised models — the interpolation threshold is where the model first achieves zero training error, which means the classical U-curve is the left half of a more complex curve.`,
      `Implicit regularisation of SGD: among all zero-training-error solutions, gradient descent finds the minimum-norm one — a form of implicit L2 regularisation, which means overparameterised models trained with SGD generalise better than classical theory predicts.`,
      `Generalisation in practice: train/val/test split hygiene — the test set must never be used for model selection, which means any decision (architecture, hyperparameters, features) based on test set performance makes the test set into a validation set and you no longer have an unbiased estimate.`,
    ],
    checkQuestions: [
      {
  ...
  },
  {
    ...
    keyPoints: [
      `Splitting criterion: find feature j and threshold t minimising weighted child impurity — Gini: G = 1−Σpₖ²; Entropy: H = −Σpₖlog pₖ. Both give very similar splits in practice; Gini is faster to compute, which means the choice rarely affects model quality but can affect speed on large datasets.`,
      `Greedy algorithm: splits are locally optimal but not globally — the globally optimal tree is NP-hard to find, which means all practical tree implementations are greedy and suboptimal by construction.`,
      `A fully grown tree memorises training data: one leaf per training sample gives 100% training accuracy — this is extreme overfitting with zero bias and enormous variance, which means depth must be limited or the tree must be pruned.`,
      `Cost-complexity pruning (sklearn default): adds penalty α × (number of leaves) to total error, grows the full tree, then prunes back leaves that increase the penalised criterion — α is selected via CV, which means pruning trades bias for variance reduction in a principled way.`,
      `Feature importance from trees: Gini importance = total impurity decrease weighted by node sample count — biased toward high-cardinality continuous features and features with many possible splits, which means permutation importance (shuffle feature, measure accuracy drop) is more reliable and less biased.`,
      `Instability (high variance): a different random seed for data splitting often gives a completely different top split, which cascades through the entire tree — this is WHY random forests work: averaging over many unstable trees produces a stable ensemble.`,
      `Trees handle non-linearity and interactions automatically: splitting on feature A then B encodes the interaction A×B, which means trees are powerful for tabular data with unknown interaction structure without any feature engineering.`,
      `Trees do not extrapolate: a tree predicts by returning the mean of the leaf it falls into — for a test point outside the training feature range, it returns the nearest leaf value (constant prediction), which means tree-based models fail when the test distribution has different support than training.`,
    ],
    checkQuestions: [
      {
  ...
  },
  {
    ...
    keyPoints: [
      ...
      `Feature subsampling: at each split, consider only m random features — m = √d for classification, d/3 for regression (sklearn defaults). This decorrelates trees beyond what bagging alone achieves, which means max_features is the hyperparameter with the most impact on the bias-variance tradeoff.`,
      `Variance reduction formula: ensemble variance = ρσ² + (1−ρ)σ²/T — as T→∞ this approaches ρσ², so the floor is determined by inter-tree correlation ρ, not tree count, which means adding more trees past ~200 has diminishing returns but reducing ρ (via smaller max_features) continues to help.`,
      `OOB error: for each sample, average predictions from trees that did NOT use it in training — approximates test error without a separate validation set, which means it is a free diagnostic and should always be checked (oob_score=True in sklearn).`,
      `Permutation importance: for each feature, shuffle its values in the OOB set and measure the drop in accuracy — unbiased and detects non-linear effects, which means it is more reliable than Gini importance for correlated features.`,
      `Gini importance is biased toward high-cardinality features: a feature with many unique values gets more split opportunities and thus more impurity-reduction credit — which means never use Gini importance for feature selection; use permutation importance or SHAP.`,
      ...
      `Random forests are embarrassingly parallel: each tree is independent, which means n_jobs=-1 in sklearn uses all cores with linear speedup — always set this on multi-core machines.`,
    ],
    checkQuestions: [
      {
  ...
  },
  {
    ...
    keyPoints: [
      `Algorithm: initialise F₀ = argmin_γ Σ L(yᵢ,γ); for t=1..T: compute pseudo-residuals rᵢ = −∂L/∂F(xᵢ)|_{F=F_{t-1}}; fit tree hₜ to {rᵢ}; update F_t = F_{t-1} + η·hₜ — which means gradient boosting is gradient descent in function space, where each tree is a gradient step.`,
      `For MSE loss: pseudo-residuals are actual residuals (yᵢ − F_{t-1}(xᵢ)) — each tree corrects what the previous ensemble got wrong. For log-loss: pseudo-residuals are (yᵢ − ŷᵢ) — same form as logistic regression gradient, which means gradient boosting generalises to any differentiable loss function.`,
      `Shrinkage (learning rate η < 1): scales each tree's contribution — smaller η requires more trees but achieves better generalisation by preventing any single tree from dominating, which means η and n_estimators always trade off: halving η requires roughly doubling n_estimators.`,
      `Boosting trees should be shallow (depth 3-8): each tree fits residuals, not the full signal — deep trees overfit the residuals, which means boosting uses shallow "stumps" as weak learners while random forests grow full-depth trees.`,
      `XGBoost objective: Σ L(yᵢ,ŷᵢ) + Σₜ Ω(hₜ) where Ω(h) = γT + (λ/2)Σⱼ wⱼ² — penalises both number of leaves (γ) and leaf weight magnitude (λ), which means these regularisation terms directly control model complexity beyond depth and n_estimators.`,
      `XGBoost split finding: uses second-order Taylor expansion L(y,ŷ+f) ≈ L(y,ŷ) + g·f + (h/2)·f² — optimal leaf weight wⱼ* = −Gⱼ/(Hⱼ+λ) in closed form; split gain formula prunes splits that do not improve the regularised objective by at least γ, which means XGBoost's regularisation is baked into the split criterion itself.`,
      `Early stopping: monitor validation loss; stop after k rounds with no improvement (early_stopping_rounds=50 typical) — this is the most important technique for finding optimal n_estimators, which means you should always use it rather than grid-searching n_estimators.`,
      `LightGBM vs XGBoost: LightGBM grows trees leaf-wise (deepest leaf first) rather than level-wise — faster for large datasets, sometimes better accuracy, but more prone to overfitting on small data, which means LightGBM is the default choice for datasets with n > 100K; XGBoost for smaller datasets.`,
    ],
    checkQuestions: [
      {
  ...
  },
  {
    ...
    keyPoints: [
      `Bagging: train T models on T bootstrap samples, aggregate by averaging (regression) or voting (classification) — reduces variance without increasing bias, which means it works best with high-variance base learners like deep trees; averaging low-variance models (linear regression) gives minimal gain.`,
      `Boosting: sequential, weighted-residual fitting — reduces bias at the cost of increased variance risk; each tree is a weak learner (high bias, low variance) that corrects the previous ensemble, which means the key risk is overfitting as trees are added.`,
      `Stacking: train diverse base models on k-fold cross-validation, use those OOF predictions as features for a meta-learner — prevents leakage from base model training data into the meta-learner, which means the meta-learner learns which base models to trust for which input regions.`,
      `Diversity principle: for an ensemble of T models each with error ε and pairwise error correlation ρ, ensemble error ≈ ε(1+(T-1)ρ)/T — ensemble is best when ρ is minimised, which means using different algorithms, different features, and different random seeds all reduce ρ.`,
      `OOF (out-of-fold) predictions for stacking: base models are trained on k-1 folds and predict on fold k — the meta-features are thus generated from held-out data, which means this is the critical step that prevents leakage from base models that have already seen the training labels.`,
      `Simple averaging vs. learned stacking: averaging base model predictions is often within 0.1% of full stacking and has zero risk of meta-learner overfitting, which means always benchmark against simple averaging before investing in a meta-learner.`,
      `Production cost of ensembles: N models to maintain, deploy, monitor, and version — latency is N× a single model at inference time, which means ensembles are almost always justified in competitions but require careful cost-benefit analysis before production deployment.`,
      `Snapshot ensembles for deep learning: save model checkpoints at different learning rate cycles, ensemble them at inference — free diversity (checkpoints are cheap), substantial accuracy gains, which means cyclical learning rate schedules provide ensemble diversity at no extra training cost.`,
    ],
    checkQuestions: [
      {
  ...
  },
  {
    ...
    summary: `SVMs find the maximum-margin hyperplane separating two classes: the hyperplane that keeps both class boundaries as far apart as possible. The margin is 2/‖w‖, so maximising margin is equivalent to minimising ‖w‖² subject to yᵢ(wᵀxᵢ+b) ≥ 1. Only the training points on or inside the margin (support vectors) determine the solution — the rest of the training data can be removed without changing the model. The kernel trick is the key generalisation: the dual formulation of SVM only requires dot products xᵢᵀxⱼ, which can be replaced by kernel function k(xᵢ,xⱼ) = φ(xᵢ)ᵀφ(xⱼ), enabling non-linear decision boundaries in the original space without computing the (potentially infinite-dimensional) feature mapping φ explicitly. SVMs have strong theoretical foundations and work well in high-dimensional spaces, but do not scale to large datasets and have been largely supplanted by gradient boosting and deep learning in practice.`,
    keyPoints: [
      `Hard-margin SVM: min ½‖w‖² s.t. yᵢ(wᵀxᵢ+b) ≥ 1 — only feasible when data is linearly separable; the dual involves dot products αᵢαⱼyᵢyⱼxᵢᵀxⱼ, which means the kernel trick only works through the dual formulation.`,
      `Soft-margin SVM: introduces slack ξᵢ ≥ 0; min ½‖w‖² + C·Σξᵢ — C is the key hyperparameter: large C = penalise violations heavily (smaller margin, may overfit); small C = allow many violations (larger margin, more robust), which means C controls the bias-variance tradeoff analogously to 1/λ in Ridge regression.`,
      `Support vectors: training points with αᵢ > 0 (on or violating the margin) — the decision function depends only on these, which means SVMs are sparse in training data (only a fraction of points matter) unlike neural networks that use all data.`,
      `Kernel trick: replace xᵢᵀxⱼ with k(xᵢ,xⱼ) in the dual — RBF k(x,x') = exp(−γ‖x−x'‖²) is an infinite-dimensional feature space (universal approximator), which means with the right kernel, SVM can learn any decision boundary.`,
      `RBF kernel γ parameter: high γ means the kernel function drops off quickly with distance (tight, localised decision boundary — overfits); low γ means wider influence (smoother boundary — may underfit), which means γ and C are jointly tuned on a log scale.`,
      `Hinge loss: max(0, 1−yᵢf(xᵢ)) — the SVM objective with soft margin is hinge loss + L2 regularisation, which means SVM and regularised logistic regression minimise different but related losses and often give similar accuracy in practice.`,
      `Practical scaling: SVMs do not scale to large datasets — O(n²) memory for the kernel matrix, O(n²-n³) training time, which means for n > 100K use linear SVM (liblinear) or SGD with hinge loss (sklearn's SGDClassifier) instead of kernel SVM.`,
      `SVM requires feature scaling: RBF kernel uses Euclidean distances — unscaled features with different ranges make the kernel meaningless, which means always StandardScaler before SVM, even more so than for most other algorithms.`,
    ],
    checkQuestions: [
      {
  ...
  },
  {
    ...
    keyPoints: [
      ...
      `Bias-variance tradeoff in k: k=1 gives zero training error (every point is its own nearest neighbour) but maximum variance; large k gives smoother, higher-bias boundaries, which means k is the regularisation hyperparameter for KNN and should be tuned via CV.`,
      `Curse of dimensionality: in d dimensions, the ratio (distance to nearest neighbour)/(distance to farthest neighbour) → 1 as d→∞, which means all points become approximately equidistant and the "neighbourhood" concept loses meaning — KNN degrades gracefully up to ~d=20, then rapidly.`,
      `Curse intuition: the fraction of volume within distance ε of a point in d dimensions shrinks as εᵈ — to maintain the same fraction of data in the neighbourhood, ε must grow exponentially with d, which means with fixed n, KNN neighbourhoods span the entire space in high dimensions.`,
      `Weighted KNN: weight votes by 1/distance — closer neighbours matter more, reducing sensitivity to the choice of k, which means weighted KNN is almost always preferable to uniform voting and should be the default.`,
      `ANN indexes: FAISS (GPU-accelerated, used in production search systems), HNSW (graph-based, excellent recall-speed tradeoff), ScaNN (Google) — reduce query time from O(nd) to O(log n) or sublinear at the cost of <1% recall loss, which means ANN is the only viable approach for n > 1M.`,
      `KNN on learned embeddings: raw features often mislead distance; KNN on embeddings from contrastive learning or fine-tuned transformers is powerful for few-shot classification, semantic search, and recommendation, which means the feature space matters more than k.`,
      `KNN for imputation: replace missing value with weighted average of k nearest complete neighbours — simple and often competitive with more complex imputation, which means it is the fastest non-trivial imputation strategy to implement.`,
    ],
    checkQuestions: [
      {
  ...
  },
  {
    ...
    keyPoints: [
      `Prediction: ŷ = argmax_k P(y=k) Π_j P(xⱼ|y=k) — use log-probabilities: argmax_k [log P(y=k) + Σⱼ log P(xⱼ|y=k)] to avoid numerical underflow from multiplying tiny probabilities.`,
      `Gaussian NB: P(xⱼ|y=k) = N(μⱼₖ, σⱼₖ²) — fits per-class mean and variance; good for continuous features; training is just computing class-conditional means and variances, which means it trains in a single pass over the data.`,
      `Multinomial NB: P(xⱼ|y=k) ∝ count of feature j in class k — standard for text classification where features are word counts/frequencies, which means it is the correct variant for document classification (not Gaussian NB which is for continuous data).`,
      `Laplace smoothing: add α=1 to all counts — P(word|class) = (count+1)/(N_class+|V|) — prevents zero probabilities from zeroing out the entire product, which means it acts as a Dirichlet prior and is essential for any vocabulary with unseen words at test time.`,
      `Why naïve NB works despite violated independence: classification requires only that P(y=k|x) is ranked correctly, not that the absolute probabilities are accurate — even if the joint distribution is badly modelled, the relative ordering of class posteriors is often correct.`,
      `Posterior miscalibration: NB probability outputs are poorly calibrated (often pushed toward 0 and 1), which means ŷ = 0.99 from NB should not be interpreted as "99% confidence" — use Platt scaling to recalibrate if calibrated probabilities are needed.`,
      ...
      `Complement NB: for text, model P(x|y≠k) and predict class with minimum complement score — corrects for class imbalance and often outperforms standard Multinomial NB on imbalanced text datasets.`,
    ],
    checkQuestions: [
      {
  ...
  },
  {
    ...
    keyPoints: [
      `Reliability diagram (calibration curve): bin predictions into M buckets; for each bin, plot mean predicted probability vs. fraction of positive samples — perfect calibration is the diagonal line y=x, which means systematic deviations (S-shape, J-shape) reveal the miscalibration pattern.`,
      `ECE (Expected Calibration Error): weighted average of |accuracy_b − confidence_b| across bins, weights = bin size — scalar summary, lower is better; MCE (Maximum Calibration Error) measures worst-bin deviation, which means ECE is the standard metric but MCE matters when worst-case reliability is critical.`,
      `Overconfidence: tree-based models and unconstrained neural networks push probabilities toward 0 and 1 — histogram of predicted probabilities is bimodal, which means random forests typically need calibration before use in probability-sensitive applications.`,
      `Platt scaling: fit logistic regression σ(af+b) on validation set where f is model's raw output — simple (2 parameters), fast, appropriate when miscalibration is monotone (sigmoid-shaped calibration curve), which means it is the first thing to try.`,
      `Isotonic regression: fits a non-decreasing step function to (score, label) pairs on the calibration set — more flexible than Platt, appropriate for non-monotone miscalibration, but needs more data (minimum ~1000 calibration samples to avoid overfitting), which means use Platt when data is scarce.`,
      `Temperature scaling for neural networks: divide logits by scalar T before softmax — T>1 softens distribution (reduces overconfidence); only one parameter fitted on validation set, remarkably effective, which means it is the standard calibration method for neural networks post-training.`,
      `Calibration vs. discrimination: a model can be well-calibrated but have poor AUC, or excellent AUC but poor calibration — they measure different things, which means you need both a discrimination metric (AUC) and a calibration metric (ECE) to fully evaluate a probabilistic model.`,
  ...
  },
  {
    ...
    keyPoints: [
      `Never use accuracy for imbalanced problems — a 99% accurate model on 1% positive rate predicts negative always; use precision, recall, F1, AUC-ROC, PR-AUC, or business-specific cost metrics, which means the evaluation metric choice is as important as the model choice.`,
      `Class weights: set class_weight ∝ 1/frequency — sklearn's class_weight="balanced" computes wₖ = n/(K·nₖ); reweights the loss function without modifying the data, which means it is the cheapest and least risky intervention to try first.`,
      `SMOTE: for each minority sample, generate synthetic samples along line segments to k nearest minority neighbours — avoids pure duplication, which means it adds interpolated examples rather than copies; the failure mode is creating synthetic samples in regions overlapping with the majority class.`,
      `Undersampling: randomly remove majority class samples — faster training, preserves class boundary information; Tomek links removes majority samples that are the nearest neighbour of minority samples (cleaning the boundary), which means combining SMOTE with Tomek link removal (SMOTEENN) is often better than either alone.`,
      `Threshold tuning: default threshold 0.5 assumes equal costs — lower threshold increases recall at the cost of precision; set threshold to optimise the business metric (e.g., F_β with β>1 for recall-heavy problems), which means threshold tuning is always required at deployment because the cost ratio of false positives to false negatives is problem-specific.`,
      `Precision@K: among the top-K highest-confidence positive predictions, what fraction are correct — relevant when you act on a fixed budget (K fraud alerts per day, K highest-risk patients), which means Precision@K is often the most business-aligned metric for imbalanced problems.`,
      `PR-AUC vs ROC-AUC: for very imbalanced data, PR-AUC is more informative — ROC-AUC can be high (0.99) even when Precision@K is terrible, because ROC-AUC is dominated by the many true negatives, which means always check PR-AUC alongside ROC-AUC for imbalanced problems.`,
      `SMOTE with tree-based models: trees handle imbalance natively with class weights (the split criterion directly weights class frequencies); SMOTE sometimes hurts trees by adding noise in regions where the tree would otherwise split cleanly, which means use class weights for trees and SMOTE primarily for neural networks and SVMs.`,
    ],
    checkQuestions: [
      {
  ...
  },
  {
    ...
    keyPoints: [
      `Filter methods: evaluate features independently of the model — mutual information I(X;y) is most general (captures non-linear dependencies), ANOVA F-test for linear relationships with continuous targets, chi-squared for categorical targets and features, which means filter methods are fast but miss multivariate interactions.`,
      `Variance threshold: remove features with near-zero variance — they cannot be predictive regardless of the model; constant features (var=0) are always removed first, which means this is the cheapest first pass and should always precede any other selection method.`,
      ...
      `Wrapper methods: RFE (Recursive Feature Elimination) trains model, removes lowest-importance feature, repeats — captures feature interactions at the cost of O(d) model fits, which means RFE is practical for small d (<100) but infeasible for d=10,000.`,
      ...
      `Feature selection bias: if you use the full dataset (including test labels) to select features, the selected features appear predictive but this predictive power is artefactual — feature selection must happen inside cross-validation folds, not before splitting.`,
      `SHAP-based selection: train one gradient boosting model on all features, compute mean |SHAP value| per feature, select top-k — SHAP values account for interactions correctly (unlike Gini importance), which means this is the most reliable selection approach for tabular data.`,
      `Collinearity removal: if two features have |corr| > 0.95, they are nearly redundant — keep the one with higher mutual information with the target and remove the other, which means correlated feature pairs inflate apparent feature count without adding predictive power.`,
    ],
    checkQuestions: [
      {

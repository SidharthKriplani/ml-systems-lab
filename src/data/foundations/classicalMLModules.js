...
Why care? Because people read the weights to decide which fact matters. See a weight near zero and think "this one does nothing, drop it," and you can be badly wrong — the weight may be tiny only because its twin grabbed the credit. The usual fix is a method called **Ridge**, which keeps the weights from growing wild and shaky by shrinking them smoothly toward zero — both correlated weights survive, just tamer. A related method, **Lasso**, does something stronger: under this same collinearity it can zero out one of the two correlated weights entirely and hand all the credit to the other, and under high correlation which one it keeps is essentially arbitrary — a small change in the data can flip which weight survives. The Regularisation module picks up exactly here — and reuses this module's own Sxy=1900 and Sxx=250 to show, in real numbers, how Ridge and Lasso each tame a weight like our slope of 7.6.
...
You need a yardstick, and the fairest one is the dumbest possible model: the one that ignores every fact about the house and just guesses the same number every time — the average price. Call it the lazy model. How wrong is the lazy model? Take each house's price, subtract the average, square it, and add it all up. On the five houses: (200−280)²+(250−280)²+(280−280)²+(310−280)²+(360−280)² = 6400+900+0+900+6400 = **14,600**. That total is simply the spread of prices around their average, and that spread has a famous name: the **variance**. (Take its square root and you are back in real dollars — that is the **standard deviation**. And the average you started from is the **mean**. Mean, then variance, then standard deviation, each one built on the one before.)
...
The trick uses a pair of functions that undo each other. You already know one such pair: eˣ and its inverse, the natural log. eˣ takes any number and gives back a positive one — its output lives in (0, ∞). The natural log runs it backwards: hand it a positive number, it gives back any number at all. Now watch a second pair do the same dance around (0, 1). Feed the number 0.5 into $1/(1 + e^{-z})$: you get 1/(1+0.6065) ≈ 0.622 — inside (0, 1). Feed it −1.6: 1/(1+4.953) ≈ 0.168 — inside (0, 1) again. Any number on the whole line goes in; a clean probability-shaped number comes out. That squashing function is the **sigmoid**, $σ(z) = 1/(1 + e^{-z})$ — and yes, it is the same S-curve Verhulst drew. It too has an inverse, running the dance backwards — stretching (0, 1) back across the whole number line: 0.622 → 0.5, and 0.168 → −1.6. That stretch is the **logit**. The squash is exactly the bend we were hunting for.
...
Reach for the obvious loss — mean squared error, the one linear regression uses — and watch it fail. Bring in two more patients, scored with the same w=1.4, b=−0.2, both of whom truly had a heart attack (y=1). Patient 3 has x=2.0: z=1.4×2−0.2=2.6, σ(2.6)≈**0.931**. The model was basically right, and the squared error shows it: (0.931−1)² ≈ **0.005** — tiny. Patient 4 has x=−2.0: z=1.4×(−2)−0.2=−3.0, σ(−3.0)≈**0.047**. The model insisted this person was low-risk — said of a patient who in fact had a heart attack. Confidently, badly wrong. Yet the squared error is only (0.047−1)² ≈ **0.908**. Push the prediction even further wrong, down to 0.0001, and MSE barely moves at all — (0.0001−1)² ≈ 0.9998, essentially the same number. MSE has a hard ceiling at 1 no matter how confidently wrong the model gets.
...
The decision boundary logistic regression draws is *linear* in whatever feature space you give it — a line, a plane, a hyperplane. That is a real limit, but also a lever: you make the model as expressive as you like by *engineering the features*. Add interaction terms (age × blood_pressure) to let features combine, polynomial or spline terms to let a feature curve, and binning to let it jump in steps. Done well, this keeps the interpretability and calibration of logistic regression while letting it fit relationships a raw straight line never could — often you reach for a heavier model only after these run out.`,
    ...
    checkQuestions: [
      ...
      {
        q: `While training on real patient data with a hand-rolled training loop, the weights keep growing and the loss eventually becomes NaN. It turns out one feature separates the sick patients from the healthy ones perfectly in the training set. What is happening, and what is the fix?`,
        options: [
      ...
      },
      {
        ...
        options: [
          ...
          `\`B) Patient 4's squared error (≈0.908) is barely worse than a near-maximally wrong prediction of 0.0001 would give (≈0.9998), because MSE has a hard ceiling near 1.\``,
    ...
    recap: [
      ...
      "**Train with log loss, not MSE.** On a confidently-wrong case (ŷ=0.047, y=1): MSE≈0.908 (capped near 1), log loss≈**3.05** (unbounded) — and the gradient gap (≈0.086 vs 0.953, from the 0.045 damping factor) is why MSE stalls.",
...
Add up the plain sizes instead (ignoring sign) — that is **L1**, also called **Lasso**. Lasso's one-step formula for a single centered feature is a soft threshold: slope = sign(Sxy) × max(|Sxy| − λ/2, 0) / Sxx — subtract λ/2 straight off the co-movement sum Sxy before dividing, and clamp at zero if that goes negative. At λ=0 this also reduces to 1900/250=7.6, matching OLS exactly. At λ=500: max(1900−250,0)/250 = 1650/250 = **6.6**, a drop of 1.0 from OLS's 7.6. Check that against Ridge at the same λ: slope = Sxy/(Sxx+λ) = 1900/(250+500) = 1900/750 ≈ **2.53**, a drop of about 5.07 from OLS's 7.6 — Lasso's absolute drop is smaller than Ridge's at this shared penalty. But push λ to 3800 (so λ/2=1900, exactly cancelling Sxy): max(1900−1900,0)/250 = 0/250 = **0** — exactly zero, no rounding. The feature is switched off completely, at a specific, finite, computable λ. Compare the two at their own natural stopping points: Ridge at λ=1900 still reports 0.884, a live if small weight; Lasso at λ=3800 reports a hard 0 — gone. That is the entire "diamond vs. circle" geometry from a picture, now as two numbers you can check with a calculator.
...
Lasso ties in a different way — but only along part of that tied line, not all of it. Restrict to the segment where both weights share the same sign, 0 ≤ weight₁ ≤ 7.6 — the segment that actually contains the corner solutions weight₁=7.6,weight₂=0 and the even split weight₁=weight₂=3.8: there, |weight₁|+|weight₂| = weight₁+weight₂ = 7.6 for every point, so the L1 penalty is exactly the same number, 7.6×λ, no matter how the 7.6 is divided on that segment. Step outside it and the tie breaks: at weight₁=−100,weight₂=107.6 — also a point on the line weight₁+weight₂=7.6 — the penalty is |−100|+|107.6| = 207.6, far larger than 7.6, because opposite-sign weights add their magnitudes instead of letting them cancel. So within the same-sign segment Lasso's objective genuinely cannot prefer one split over another — unlike Ridge's bowl-shaped penalty, which is curved everywhere, Lasso's penalty is flat only on that segment, so there is no unique minimum to fall back on there (and it's strictly higher everywhere outside it, so a solver never wanders out to points like weight₁=−100,weight₂=107.6 anyway). In practice a solver (coordinate descent) breaks that flat tie arbitrarily within the same-sign segment, typically landing on a corner: weight₁=7.6, weight₂=0, or the reverse, whichever column happens to get updated first. That is the mechanism behind "which correlated feature Lasso keeps can flip from one run to the next" — it isn't a bug, it's what an unpenalised flat direction in the loss does to any greedy solver.
...
Finally, regularisation isn't a linear-regression trick — it's everywhere. It's the \`C\` in logistic regression and SVMs, the margin-softening in SVMs, and **weight decay** in neural networks (L2 on the network's weights). The penalty interacts differently with each loss and solver, but the core move is identical: add a cost on complexity so the optimiser stops chasing the training noise.`,
    ...
    recap: [
      ...
      "**Duplicate/correlated feature (weight₁+weight₂=7.6, tied):** Ridge deterministically splits evenly (**3.8, 3.8** — minimum-norm tie-break); Lasso's flat penalty along the same-sign segment of that tied line has no unique minimum, so a solver arbitrarily lands on a corner (**7.6, 0**) — why Lasso's picks flip across runs.",
...
Here is the setup that makes it real, with numbers you can check yourself rather than take on faith — the same scattered-vs-off-center pattern from the dartboard, now pinned to a number instead of a picture. Say you're predicting house prices, and you track two houses: a **training house** that actually sold for $300,000, and a **held-out house** — never shown to the model during training — that actually sold for $250,000.

Train an overly flexible model (lots of features, little regularisation) on the training set, and it predicts $306,000 for the training house: a $6,000 miss, i.e. 6,000 / 300,000 = **2%** error. It looks great. But feed it the held-out house and it predicts $200,000: a $50,000 miss, i.e. 50,000 / 250,000 = **20%** error. Training error 2%, test error 20% — check the arithmetic yourself, it's just two divisions. In production this model is useless, and nothing in the training loop warned you — it was doing its job perfectly on the data it could see. That 18-point gap is the real problem, and to fix it you have to know *why* it is there.

Before you read on: given a 2% training error and a 20% test error, would you expect this model's problem to be bias or variance? Keep your answer in mind — the decomposition below will tell you which, and a second model on these same two houses will get the opposite diagnosis.
...
That is also why the two failures need opposite fixes, and why guessing wrong wastes weeks. Training error high *and* test error high (say 15% and 18% — picture a rigid straight-line model on those same two houses: it predicts $255,000 for the $300,000 training house, a $45,000 miss, 45,000/300,000 = 15%, and $205,000 for the $250,000 held-out house, a $45,000 miss, 45,000/250,000 = 18%)? That is **high bias** — the model cannot even fit the data it trained on, so give it more power (more features, a more flexible model, less regularisation); more data will not help. Training error low but test error much higher (2% and 20% — our flexible model above)? That is **high variance** — the model memorised the training set, so rein it in (more regularisation, a simpler model, or the most reliable fix, more training data).

There's a way to see this diagnosis directly instead of computing it from two numbers: plot a **learning curve** — training error and validation error, both plotted against how much training data you've used, from a handful of houses up to your full set. Two shapes tell two different stories. If both curves climb up to sit high and close together — train error crawling toward that 15%, validation toward 18%, the rigid straight-line model's numbers from above — you're underfitting: the model is too weak to fit even the data it can see, and no amount of extra houses closes a gap that was never about data. If training error stays down near that 2% while validation error hovers stubbornly up near 20% and the gap refuses to close as you add houses, you're overfitting: the persistent gap itself is the tell, and it's the same variance problem the numbers above already diagnosed.

---

**How much power is too much? (going deeper)**

There is a classical way to measure a model's raw capacity to overfit, called the **VC dimension**. In plain terms it is the largest number of points the model can label *any way you like* and still fit perfectly. A straight line in a plane can do this for some 3 points but not 4 — so its capacity is small. Pile on features and the capacity climbs, and the rule of thumb is stark: the more capacity you add, the more data you need just to keep the same gap between training and test — and the data you need climbs sharply, not gently, as capacity grows. That is the formal price of flexibility.

And there is a modern twist that broke the old picture. The classic story says test error follows a U as you add capacity — too little underfits, too much overfits, one sweet spot in the middle. The worst point on that U sits right at the **interpolation threshold** — the exact capacity where the model has just enough parameters to memorise (fit exactly, zero training error on) the training set, landing on a jagged, unstable fit that chases every quirk of the data. That U is real, but it is not the whole story. Keep pushing capacity *far* past that interpolation threshold — into the giant, over-parameterised models behind today's deep learning — and test error, surprisingly, often starts falling *again*. This is **double descent**. Roughly: when a model has far more capacity than it needs, gradient descent tends to settle on the *smoothest* fit among the many that work, and smooth fits generalise well.
...
We said VC dimension is "the most points you can label any way and still fit." The exact word is **shatter**: a model *shatters* a set of points if, for *every* possible labeling of them, it can fit them perfectly. The VC dimension is the size of the largest set it can shatter — a line in 2D shatters *some* 3 points (three in general position; three points crammed onto one straight line can't be shattered, since the middle point's label is forced either way) but no set of 4, so its VC dimension is 3. Two cautions interviewers probe. First, VC dimension is *capacity*, and it is **not** the same as parameter count — some models with few parameters have huge capacity and vice versa. Second, the VC generalization bound (gap ∝ √(VC/n)) is *conceptually* central but *numerically loose* — it's a useful way to think, not a number you'd quote to predict real test error.
...
Every guarantee here — bias-variance, VC bounds, PAC, even the honest test set — quietly assumes train and deployment data come from the *same* distribution. Production breaks that constantly, which is why a model can look great in validation and fail live. **Covariate shift**: the inputs P(X) drift (new user demographics) while the true relationship holds. **Concept drift**: the relationship P(Y|X) itself changes (fraud tactics evolve). **Train-serving skew**: a feature is computed differently in training than in serving. When validation looked good but production didn't, this family — not bias or variance — is usually the culprit; the monitoring taxonomy module covers how to detect and alert on each of these once you're live.`,
    keyPoints: [
...
Pause and predict: two candidate questions are on the table, "is income below 48k?" and "is debt-to-income at or above 0.325?" — both come from the standard way a tree generates candidates: sort each feature's values and try the midpoint between every adjacent pair, so 48k sits between two neighbouring incomes and 0.325 between two neighbouring debt ratios, not picked by hand. Which one do you expect splits these eight people more purely?
...
Now try debt. At or above 0.325: Ann, Bob, Cid, Eve — every one of them defaulted, Gini = **0** (perfectly pure). Below 0.325: Dee, Fay, Gus, Hal — every one of them stayed safe, Gini = **0** (perfectly pure too). Weighted Gini after this split: 0. The debt question separates all four defaulters from all four safe applicants in one move — Dee's low income didn't matter, Eve's high income didn't matter, only the debt ratio decided their fate correctly. It wins by the full margin available (0.5 → 0), against income's partial win (0.5 → 0.375), so the tree keeps it as the root.

Both children are already pure, so the tree stops after one split. The resulting tree has exactly two leaves: **debt ≥ 0.325 → predict default (4 of 4 training rows, 100%)**; **debt < 0.325 → predict safe (0 of 4, 0%)**. That's the whole training algorithm on this data: at every node, try every candidate question, keep the one that purifies the most, recurse until a group is pure or too small to split.

---

**What a leaf says.**

A leaf just reports the mix of training points that landed in it — the debt≥0.325 leaf above says "100% chance of default" because all four training rows there defaulted. For predicting a number instead of a class — a loan amount, say — a regression leaf hands back the **average** of the training values that landed in it instead of a class vote.
...
A classification leaf reports the *frequency* of each class among its training points. The debt-split tree above said "100% chance of default" and "0% chance of default" from its two leaves — and that's exactly the failure mode to distrust: each leaf held only four training rows, so "100%" really means "4 out of 4 seen so far," not "certainty." A single deep tree tends to give overconfident near-0/near-1 probabilities precisely because small, pure-looking leaves are easy to produce and easy to over-trust. If you need trustworthy probabilities from a tree, enforce a minimum leaf size and calibrate (Platt or isotonic) on a held-out set rather than trusting the raw leaf fractions.`,
    keyPoints: [
      `**What a decision tree is, and when to reach for it: a flowchart of yes/no questions you can actually read.**\n\nTrees are the model to use when you need to explain every prediction in plain words — "debt-to-income at or above 0.325, so we flagged it." They take mixed feature types (numbers and categories) as they come, need no scaling, and pick up feature interactions on their own, since splitting on income and then on debt is exactly an income-and-debt rule. The catch: a single tree is twitchy and overfits easily. So use one tree when you need a human-readable explanation, and an ensemble (random forest or boosting) when you need the accuracy in production.`,
...
`Gradient boosting keeps AdaBoost's core move — train sequentially, each tree fixing what's left over — but rebuilds the "fixing" step around something more general than reweighting rows: fitting to a **residual**. Watch it happen on numbers you can check by hand.`,
...
Exact kNN costs **O(nd)** per query. Approximate indexes bring the *asymptotic* shape down to **O(d log n)**, with a real constant — the search-width parameter ef — that turns the textbook number into the one production systems actually spend. For this catalog (n = 10,000,000, d = 256): brute force is n × d = 2,560,000,000 ops; the O(d log n) shape alone is d × log2(n) ≈ 5,952; the real ANN cost with ef ≈ 100 is ef × d × log2(n) ≈ 595,200; and a rerank pass over the top 200 candidates adds another 51,200. Four numbers, all checkable with the same multiplication and the same catalog — nothing here is asserted without the arithmetic behind it.`,
    ...
    },
  {
    ...
    summary: `An email arrives: "Claim your FREE prize NOW." You need to classify it as spam or ham in milliseconds. You have word frequency statistics from training: P(FREE|spam) = 0.45, P(FREE|ham) = 0.01, P(prize|spam) = 0.32, P(prize|ham) = 0.002, P(Claim|spam) = 0.18, P(Claim|ham) = 0.04. Bayes' theorem gives the posterior: P(spam|words) ∝ P(spam) × P(FREE|spam) × P(prize|spam) × P(Claim|spam) × .... You multiply across all words in the email. Whichever class — spam or ham — gives the larger product wins. Assuming equal priors, spam's product here is 0.45 × 0.32 × 0.18 ≈ 0.0259; ham's is 0.01 × 0.002 × 0.04 ≈ 0.0000008 — spam wins by roughly four orders of magnitude, an easy call even before the class prior P(spam) is folded in.
...
The three variants handle different data types. Multinomial NB uses word counts, treating each email as a bag of word draws from a class-conditional multinomial — the right choice for text. Bernoulli NB uses word presence or absence, ignoring count information — faster but less informative; it tends to win on short texts (tweets, subject lines) where a word rarely appears more than once, so counting adds noise rather than signal. Gaussian NB models continuous features as Gaussian distributions per class, fitting one mean and one variance per feature per class. For the spam filter, Multinomial NB is correct. For a dataset with continuous medical measurements, Gaussian NB is correct. Applying Gaussian NB to text, or Multinomial NB to continuous features, produces silently wrong models.

**NOT this.** Naive Bayes is too simple for real use. For high-dimensional sparse features — text, categorical bags — Naive Bayes is competitive with SVMs and logistic regression while training in milliseconds on a single pass through the data. With 10,000 vocabulary items and 100 training documents, logistic regression has 10,000 parameters to estimate and overfits severely even with strong regularization. Naive Bayes has 20,000 simple count estimates (one P(word|class) per word per class, spam and ham) that are stable at any sample size. It is still deployed in production spam filters and intent classifiers where training speed, interpretability, and stability with small data matter more than 1–2 percentage points of accuracy against a tuned neural classifier.
...
The formal statement: ŷ = argmax_k [log P(y=k) + Σⱼ log P(xⱼ|y=k)]. Always compute in log-space. Multiplying thousands of probabilities like 0.001 causes floating-point underflow to exactly zero before the product completes. Log-space turns the product into a sum, eliminating underflow entirely.`,
    ...
    interactivePrompt: `Before you touch the controls: raise the prior P(H) and watch the posterior shift even before any evidence comes in — then raise the sensitivity P(E|H) instead and watch how much less the prior alone matters. Which one ends up dominating the final answer, the prior or the evidence?`,
    checkQuestions: [
      {
        ...
        options: [
          ...
          `\`D) The zero probability also drives ham's log-posterior to negative infinity in the same step, so the comparison becomes undefined and NB cannot classify the email at all.\``,
        ],
        answer: ['A', 'B'],
      },
      {
...
Here is that arithmetic worked through once, concretely. Say the 0.7-0.8 bucket contains 300 predictions with an average predicted probability of 0.75, and 180 of those 300 actually turned out positive — an actual rate of 0.60. The gap for that bucket is |0.75 − 0.60| = 0.15, weighted by its 300/N share of all predictions. Summing that weighted gap across every bucket gives the Expected Calibration Error (ECE) — one number for how far the whole curve sits from the diagonal.
...
Finally, calibration is not permanent. A model calibrated on last year's data can drift out of calibration as the world changes — **covariate shift** (the input mix moves) or **concept drift** (the relationship changes) both break it, even though your original test-set calibration looked perfect. So calibration is something to *monitor* in production (track ECE or reliability over time), not a one-time fix at training. When a deployed model's probabilities start lying, drift is the usual cause.`,
    ...
    takeaway: `AUC tells you if a model ranks cases correctly; calibration tells you if its probabilities are actually true — when it says 0.7, does it happen 70% of the time? The two are separate, and most models come out miscalibrated by default — neural nets typically overconfident, random forests typically pushed toward the middle (under-confident at the extremes). Whenever a decision uses the probability itself, plot the reliability diagram, and fix miscalibration with Platt scaling or isotonic regression on a separate calibration set — never on training or test.`,
    recap: [
      ...
      "**Most models come out miscalibrated by default** — neural nets overconfident, random forests pushed toward the middle (under-confident at extremes).",
...
**Filter methods** are the quick screen. Before anything model-based, the cheapest possible pass costs no training at all: drop constant or near-constant features (they carry no signal) and, from each highly correlated pair, drop one (the other carries almost the same information) — a free first cut. Beyond that zero-cost screen, filter methods score each remaining feature on its own — how strongly does it relate to the target, using a measure like correlation or mutual information — and keep the top ones. Fast and simple, but blind: they judge each feature in isolation, so they can throw away two features that are useless alone yet powerful together.

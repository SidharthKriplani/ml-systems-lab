...
Here is a job where it shines and a straight-line model struggles. Say you want to flag loan applicants likely to default, using their income and their debt-to-income ratio. The real pattern is a set of rules: "if income is low *and* debt is high, risky — but a big income excuses a fair bit of debt." That is not a smooth weighted sum. It is the space of applicants carved into regions, each with its own answer. A linear model draws one line and gives up; a tree carves — in general, by stacking multiple splits into a multi-region boundary no single line can match, though (as you'll see) the specific eight-applicant numbers below turn out simple enough to resolve with just one split.

[FIGURE: tree_partition]

*(That figure illustrates the general case — a tree stacking several splits into multiple rectangular regions. The specific eight-applicant walkthrough below is simpler: it turns out to resolve in a single split on debt alone, so don't expect three income bands to fall out of the numbers that follow.)*
...
A classification leaf reports the *frequency* of each class among its training points. The debt-split tree above said "100% chance of default" and "0% chance of default" from its two leaves — and that's exactly the failure mode to distrust: each leaf held only four training rows, so "100%" really means "4 out of 4 seen so far," not "certainty." A single deep tree tends to give overconfident near-0/near-1 probabilities precisely because small, pure-looking leaves are easy to produce and easy to over-trust. If you need trustworthy probabilities from a tree, enforce a minimum leaf size and calibrate (Platt or isotonic) on a held-out set rather than trusting the raw leaf fractions — the \`calibration\` module ahead covers both methods in depth, with its own worked numbers.`,
  ...
  <text x="180" y="187" text-anchor="middle" fill="var(--ink-low)" font-size="8">each split is one yes/no question → axis-aligned (straight, one-feature-at-a-time) cut</text>
</svg>`,
    },
  },
  {

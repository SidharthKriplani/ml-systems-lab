...
Here is the algorithm, concretely. Step 1: initialise $F_0(x)$ as the best constant prediction — for MSE, that is the training mean; for log-loss, that is $\\log(p/(1-p))$ where $p$ is the base rate. Step 2: compute the residual for every training example — how far off is $F_0$ on each loan? Step 3: fit a shallow tree to those residuals (not the original labels — to the error). Step 4: add the tree to the ensemble, scaled by $η$: $F_1(x) = F_0(x) + η \\cdot h_1(x)$. Step 5: compute new residuals. Repeat steps 3–5 for $T$ iterations. The first tree patches the biggest systematic errors. The second patches the biggest remaining errors. Each tree is a corrector, not a predictor.

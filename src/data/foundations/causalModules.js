...
None of this means causal inference is impossible without perfect data. It means every causal number carries assumptions the same data can't verify — SUTVA, consistency, and positivity are claims about how the world generated the data, not properties a p-value certifies. Argue them from domain knowledge, design them away through randomization, or bound them with sensitivity analysis — never assume they hold just because the difference in means came out significant.`,
    ...
    recap: [
      ...
      `**Classic estimand error:** using the ATT — the effect among people who *self-selected* into treatment — to justify a *universal* rollout, when the self-selected group may benefit far more than the average user who'd receive it.`,
...
What this is not: "control for everything." Controlling for a collider creates bias where none existed before. Controlling for a mediator blocks the path you want to measure. Controlling for a post-treatment variable that is a descendant of Treatment can do both. You need the DAG to know which variables to condition on and which to leave out. A "control for everything" strategy without a DAG is a systematic way to introduce collider bias while believing you removed confounding.`,
    ...
    checkQuestions: [
      ...
      },
      {
        ...
        options: [
          `A) No — a negative coefficient is expected because high performers tend to be newer hires with fewer years of experience, so the sign is just a compositional artifact of tenure, not a real effect worth investigating further`,
          `B) No — the negative sign reflects multicollinearity between experience and performance, which inflates the standard errors on both coefficients but leaves the underlying point estimates themselves technically unbiased`,
          `C) Yes — a sign of collider bias: both experience and performance cause salary, so conditioning on salary opens a spurious negative path between them within a fixed band, not evidence high performers earn less`,
          `D) Yes — the negative sign signals severe omitted-variable bias from some unmeasured confounder, and the recommended fix is to keep adding control variables until the performance coefficient eventually turns positive`,
        ],
        answer: `C`,
      },
    ],
    ...
    recap: [
      ...
      `**Mediator** (T → M → Y): sits on the causal path you're trying to measure, so conditioning on it *blocks* that path and *underestimates* the total effect — the opposite of what you want.`,
      ...
      `**Diagnostic:** if adding a control shifts the estimate by more than ~50%, it's either a real confounder (good — expected) or a collider you just opened (bad) — draw the DAG to tell which. Arrows coming *into* it from both T and Y = collider, remove it.`,
    ],
    figures: {
...
None of this is what "statistically significant" certifies. p = 0.02 says the gap is unlikely under no effect — nothing more. That reading only holds if randomization was valid, the primary metric was pre-specified, the run lasted as planned, and the result is worth acting on. A p = 0.001 result on a 0.01% lift costing \$500K to build isn't a success story — significance means reliably nonzero, not worth building.`,
    ...
    recap: [
      ...
      `**Unit of randomisation is a design trade-off:** user-level gives each user a consistent experience and low contamination; session-level gives more power but the same user can see both variants; page-level has the most power and the highest contamination risk.`,
      ...
      `**Run an AA test to validate the randomisation:** split the control group in two and test for a difference on the primary metric — a significant one means the assignment infrastructure is biased and no AB result from it can be trusted.`,
      `**Statistical significance ≠ success:** p = 0.001 tells you the estimate is reliably nonzero, not that it's worth acting on — a 0.01% effect costing \$500K to ship is a significant failure.`,
    ],
  },
  {
    id: 'observational_ci',
...
What this is not: any variable correlated with treatment is a valid instrument. The exclusion restriction is almost never testable. A weak instrument (first-stage F < 10) produces estimates with enormous variance and finite-sample bias toward OLS that defeats the purpose of the IV approach entirely. A good instrument is extremely hard to find — this is why natural experiments are so valuable in applied causal inference.`,
    ...
    recap: [
      ...
      `**Diagnostic — compare IV to OLS:** the direction of the gap reveals the direction of the unmeasured confounding (IV > OLS ⇒ the confounder was downward-biasing OLS; opposite signs ⇒ strong confound or a violated exclusion restriction — investigate before publishing).`,
    ],
    figures: {
...
What RDD requires that must always be checked: units cannot precisely manipulate which side of the cutoff they land on. If students can adjust their score to land just above 70, the units just above are not comparable to units just below — they are systematically different in their ability or motivation to game the system. Always test for bunching in the running variable distribution using the McCrary density test. Significant bunching at or just above the cutoff means the local randomization assumption is violated.`,
    ...
    recap: [
      ...
      `**Fuzzy RDD:** treatment probability jumps only from p to p′ (not all the way), so use the threshold indicator as an *instrument* (IV) and estimate the LATE for compliers at the cutoff.`,
...
What a response model is not: an uplift model. A response model predicts P(purchase) — dominated by Sure Things. An uplift model predicts P(purchase | treatment) − P(purchase | no treatment) — targets Persuadables. These are orthogonal quantities. Deploying a response model as an uplift model wastes marketing budget and misses the actual causal effect of the treatment.`,
    ...
    checkQuestions: [
      {
        ...
        options: [
          ...
          `B) Some high-propensity customers may be Do-Not-Disturbs — loyal buyers who get annoyed by discount emails and eventually unsubscribe — so treating them can actively backfire rather than merely waste budget`,
    ...
    recap: [
      ...
      `**The meta-learners:** S-learner (include T as a feature, one model), T-learner (fit treated and control separately, subtract), X-learner (impute individual effects and borrow strength across groups), causal forests (non-parametric, with valid confidence intervals).`,
      ...
      `**Evaluate with a Qini curve / AUUC** — rank users by predicted uplift and measure cumulative incremental outcome vs random — but a no-contact holdout A/B group is *required* to confirm the model targets causal heterogeneity, not baseline propensity.`,

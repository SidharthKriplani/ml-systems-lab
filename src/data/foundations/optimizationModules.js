...
Random mini-batching assumes every batch is a fair sample — but under heavy imbalance a batch of 32 from a 0.1%-positive dataset frequently contains *zero* positives, so many steps carry no signal about the rare class. Fixes: **stratified batches** (force a minimum number of rare-class examples per batch), **weighted sampling** (oversample the rare class into batches), and **hard-example mining** (bias batches toward the examples the model currently gets wrong). Under imbalance, how you *build* the batch matters as much as its size.`,
    ...
    checkQuestions: [
      ...
      },
      {
        ...
        options: [
          `\`A) Full-batch has a noisy gradient — averaging a million examples paradoxically adds sampling error — and 1 update per epoch; mini-batch gives a mathematically exact gradient for its 256 examples and 3,906 updates per epoch, so mini-batch is the accurate one per step.\``,
          ...
          `\`C) Full-batch: exact gradient (zero noise) but only 1 update per epoch — slow, a million forward passes buy one step. Mini-batch (B=256): noisy gradient but 3,906 updates per epoch; less accurate per step, yet far more steps, reaching a good solution faster.\``,
          `\`D) Full-batch is exact with 1 update per epoch; mini-batch has 3,906 updates but non-monotone convergence. For a million examples, full-batch is always better, because at that scale the landscape becomes effectively convex and the noise buys absolutely nothing.\``,
        ],
        answer: `C`,
      },
      {
    ...
    checkQuestions: [
      ...
      },
      {
        ...
        options: [
          ...
          `\`B) Vanilla momentum applies velocity at the current θ: v ← βv + ∇L(θ). Nesterov computes it at the look-ahead θ' = θ − αβv instead, seeing what the gradient looks like after the step. Vanilla overshoots and oscillates near minima; Nesterov brakes early, hitting the optimal O(1/t²) rate versus O(1/t).\``,
    ...
    checkQuestions: [
      {
        ...
        options: [
          ...
          `\`C) At t=1: m_1=0.1·g_1, v_1=0.001·g_1², so the uncorrected step is α·(0.1g)/√(0.001g²) = α·3.16·sign(g) — 3.16x the intended scale. With correction, the step is exactly α·sign(g_1). At t=10 the uncorrected moments are still biased by ~1.54x. Since gradients are often largest at initialization, amplifying these early steps risks divergence or a bad early basin.\``,

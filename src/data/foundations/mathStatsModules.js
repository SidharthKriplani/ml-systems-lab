...
**NOT this.** Most people think $P(A|B) = P(B|A)$. They do not. A test that correctly identifies 99% of sick patients — $P(positive|disease) = 0.99$ — does not mean a positive result means you are 99% likely to have the disease. If the disease affects only 1 in 1000 people, then $P(disease|positive) \\approx 9\%$. The false positive rate, applied to the large healthy population, swamps the true positives. Dropping the prior — treating $P(positive|disease)$ as $P(disease|positive)$ — is the error. The formal rules: conditional probability $P(A|B) = P(A \\cap B)/P(B)$. Independence: $P(A \\cap B) = P(A)P(B)$. Chain rule: $P(A,B,C) = P(A)P(B|A)P(C|A,B)$.`,
    ...
    recap: [
      ...
      "**Report precision at the real base rate**, not on a balanced test set — 99% precision + 0.1% fraud ≈ 9,900 false alerts per 10k.",
    ...
    checkQuestions: [
      {
        ...
        options: [
          `A) C=1/2, E[X]=4. Find C: Σ_{k=1}^∞ C·(1/2)^k = C·1 = 1, so C=1/2 — this treats the geometric series Σ(1/2)^k as summing to 2 instead of 1, an off-by-factor error common when the index starts at k=1. E[X] = Σ k·(1/2)^{k+1} = 4, carrying the extra (1/2) factor through the shifted-index derivative identity.`,
          ...
          `D) C=2, E[X]=2. Find C: Σ_{k=1}^∞ C·(1/2)^k = 1 requires C=2, since Σ(1/2)^k=1/2 by the geometric series formula a/(1−r) with a=(1/2), r=(1/2). E[X] = Σ k·(1/2)^k, dropping C since a normalizing constant cancels in the mean, giving (1/2)/(1/4) = 2 — matching the value expected for a rapidly decaying PMF.`,
        ],
        answer: `C`,
      },
      {
    ...
    checkQuestions: [
      ...
      },
      {
        ...
        options: [
          ...
          `C) Σ controls the shape and orientation of equal-probability contours: {x:(x−μ)ᵀΣ⁻¹(x−μ)=c}, axes along Σ's eigenvectors, radii ∝ √eigenvalues. Diagonal Σ: axes align with coordinates, uncorrelated (independent for Gaussians). Dense Σ: off-diagonals rotate axes toward eigenvector directions of max/min variance.`,
...
**NOT this.** Most people think cross-entropy is just a loss function that happens to work well. Cross-entropy is an information-theoretic quantity measuring encoding efficiency. When the model is perfectly calibrated, cross-entropy equals entropy — no wasted bits. The loss function framing hides why cross-entropy is the *right* choice, not merely a convenient one. It also hides the direction of KL: forward KL ($KL(p \| q)$, used in training) forces the model to cover all modes of the true distribution. Reverse KL ($KL(q \| p)$, used in VAEs) lets the model collapse to one mode. These produce qualitatively different learned distributions — mode-covering versus mode-seeking.`,
    ...
    checkQuestions: [
      ...
      },
      {
        ...
        options: [
          `A) D_KL(P||Q) = Σ P(x)·log(P(x)/Q(x)) = E_P[log(P(x)/Q(x))]. Asymmetric: D_KL(P||Q) ≠ D_KL(Q||P) in general, equal only when P=Q. In ML: minimising forward KL (P_data||Q_model), used in MLE, forces Q to cover all modes where P_data is nonzero (mode-covering). Minimising reverse KL (Q_model||P_data), used in VAEs, lets Q collapse to one mode (mode-seeking).`,
    ...
    recap: [
      ...
      "**Use cross-entropy, not MSE, for classification:** confident wrong 0.99 → $-\\log(0.01)\\approx 6.6$ bits; MSE ≈ 0.0001.",
      "**KL direction matters:** forward KL (MLE) is mode-covering; reverse KL (VAEs) is mode-seeking / collapsing.",
      "**Mutual information $I(X;Y)=H(X)-H(X|Y)$** detects nonlinear dependence correlation misses; $I=0$ ⟹ truly independent.",
    ],
    interactiveId: 'information_theory_viz',
  },
  {
...
[FIGURE:rank]`,
  ...
  <defs><marker id="m1" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6" fill="var(--ink-low)"/></marker></defs>
    ...
    recap: [
      ...
      "**Near-singular XᵀX (κ > 1e10) = multicollinearity;** Ridge adds λI to make it invertible — the min-norm fix.",
    ],
  },
  {
    ...
    checkQuestions: [
      {
        ...
        options: [
          `A) U is m×r (left singular vectors), Σ is r×r diagonal (nonzero singular values only), Vᵀ is r×n (right singular vectors) — this is the economy/thin SVD. The full SVD pads U to m×m and V to n×n with extra columns spanning the null spaces. Mᵣ = UΣVᵀ at these dimensions reconstructs M exactly, since rank(M)=r.`,
...
**NOT this.** Most people think "PCA removes correlated features." PCA does not select a subset of original features. It creates entirely new features that are linear combinations of *all* original features. The new features — principal components — are uncorrelated with each other by construction. But each PC mixes every original feature together. You cannot look at a principal component and say "this is pixel 4,512." The confusion matters because PCA cannot be used for feature selection if you need interpretability in the original feature space.`,
    ...
    recap: [
      ...
      "**Scale sensitivity is the trap:** dollars vs cents inflates variance 10,000×; always z-score first.",
...
[FIGURE:outer]`,
    ...
    checkQuestions: [
      ...
      },
      {
        ...
        options: [
          `A) ∂/∂W tr(WᵀAW) = AW + AᵀW = 2AW (since A is symmetric). This follows from the product rule: d(WᵀAW) = (dW)ᵀAW + WᵀA(dW), so tr(d(WᵀAW)) = tr(WᵀA dW) + tr(WᵀA dW) = tr(2WᵀA dW), giving gradient 2AW — leaving the raw sum AW+AᵀW unsimplified in the first line despite already knowing A=Aᵀ.`,
  ...
  <text x="8" y="94" fill="var(--ink-low)" font-size="7.5">shape of gradient always matches shape of W — the layout check that catches silent bugs</text>
    ...
    recap: [
      ...
      "**Backward-pass mismatch with autograd? Try transposing the Jacobian** — 80% of manual bugs are layout errors (right values, wrong shape).",
    ],
  },
  {
...
**NOT this.** Non-convex optimization is not practically hopeless. Modern neural network training is non-convex optimization that works reliably in practice because of a structural property of overparameterized loss surfaces: most critical points are saddle points, not local minima, and most local minima have similar loss values to the global minimum. The theory of why gradient descent on non-convex neural networks generalizes well is still being developed — but empirically, the non-convexity is not the obstacle it appears to be in theory.`,
    ...
    recap: [
      ...
      "**Convexity buys a unique global min, guaranteed GD convergence, strong duality, and provable rates.**",
      "**Rates:** O(1/t) for gradient descent, O(1/t²) for Nesterov acceleration.",
    ...
    checkQuestions: [
      {
        ...
        options: [
          ...
          `C) Separately, the colleague's phrasing also conflates power with significance: p=0.048 reflects the Type I error threshold crossed, not 1−β (power). Power cannot be computed from a p-value alone — it depends on the true effect size and the sample size, quantities the p-value alone can never reveal. A cleaner statement would simply be 'we reject the null at α=0.05,' with no confidence language attached at all.`,
...
[FIGURE:map]`,
    ...
    checkQuestions: [
      {
        ...
        options: [
          `A) MLE: maximise log-likelihood ℓ(μ) = −n/2·log(2πσ²) − (1/2σ²)Σ(xᵢ−μ)². Setting ∂ℓ/∂μ = 0 gives Σ(xᵢ−μ) = 0, so μ_MLE = x̄. This equals the sample mean. The result shows MLE for Gaussian mean equals least squares: minimising Σ(xᵢ−μ)² gives the same estimate, and this equivalence is exactly why MSE loss corresponds to an assumption of Gaussian-distributed noise in the residuals.`,
      ...
      },
      {
        ...
        options: [
          ...
          `D) The Beta(α,β) posterior also has mean (s+α)/(n+α+β), distinct from the MAP mode (s+α−1)/(n+α+β−2); the two coincide only in special cases such as α=β=1 (uniform prior) or in the n→∞ limit. Reporting the posterior mean instead of the mode gives a different but equally valid Bayesian point estimate — choosing between MAP and posterior mean is a modeling decision, not something MAP itself requires.`,
        ],
        answer: ['A', 'D'],
      },
      {
  ...
  <text x="10" y="100" fill="var(--ink-low)" font-size="7.5">prior pulls MLE toward 0.5 — exactly L2 regularisation. More data ⟹ likelihood dominates.</text>
    ...
    recap: [
      ...
      "**Adding $\\log P(\\theta)$ = adding regularisation:** Gaussian prior → L2/Ridge (λ=1/2τ²); Laplace prior → L1/Lasso.",

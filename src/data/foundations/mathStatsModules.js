...
**NOT this.** Most people think $P(A|B) = P(B|A)$. They do not. A test that correctly identifies 99% of sick patients — $P(positive|disease) = 0.99$ — does not mean a positive result means you are 99% likely to have the disease. Suppose the disease affects only 1 in 1000 people ($P(disease) = 0.001$) and the test also has a 1% false-positive rate ($P(positive|no\\ disease) = 0.01$, i.e. 99% specificity). Then $P(positive) = 0.99 \\times 0.001 + 0.01 \\times 0.999 \\approx 0.0110$, so $P(disease|positive) = 0.00099 / 0.0110 \\approx 9\\%$. The false positive rate, applied to the large healthy population, swamps the true positives. Dropping the prior — treating $P(positive|disease)$ as $P(disease|positive)$ — is the error. The formal rules: conditional probability $P(A|B) = P(A \\cap B)/P(B)$. Independence: $P(A \\cap B) = P(A)P(B)$. Chain rule: $P(A,B,C) = P(A)P(B|A)P(C|A,B)$.`,
    interactivePrompt: `Before you touch the controls: if a test is 99% accurate and the disease has 0.1% prevalence, what do you think the probability of actually having the disease is after a positive result — 99%, 50%, or something surprisingly low?`,
    keyPoints: [
      ...
      `**Counting orderings: the binomial coefficient.** Flip a biased coin three times with $P(H) = 0.7$. "Exactly 2 heads" can happen as HHT, HTH, or THH — three distinct orderings, each with probability $0.7^2 \times 0.3^1 = 0.147$. There are $\binom{3}{2} = 3$ such orderings (choose which 2 of the 3 flips are heads), so $P(\text{exactly 2 heads}) = 3 \times 0.147 = 0.441$. This is the binomial PMF: $P(k\ heads\ in\ n\ flips) = \binom{n}{k}p^k(1-p)^{n-k}$ — count the orderings, then multiply by the probability of any single ordering.`,
    ],
    checkQuestions: [
      {
...
**NOT this.** Most people think cross-entropy is just a loss function that happens to work well. Cross-entropy is an information-theoretic quantity measuring encoding efficiency. When the model is perfectly calibrated, cross-entropy equals entropy — no wasted bits. The loss function framing hides why cross-entropy is the *right* choice, not merely a convenient one. It also hides the direction of KL: forward KL ($KL(p \| q)$, used in training) forces the model to cover all modes of the true distribution. Reverse KL ($KL(q \| p)$) is mode-seeking. A VAE's ELBO decomposes as $\log p(x) = \text{ELBO} + KL(q(z|x)\|p(z|x))$, so maximising the ELBO implicitly minimises the reverse KL between the approximate and true posterior — that's what lets the approximate posterior concentrate on a subset of the true posterior's modes rather than spreading across all of them. (The ELBO's explicit $q(z|x)$-to-prior term is a separate regulariser toward $p(z)$, not the mechanism behind this mode-seeking behaviour.) In VAEs this shows up as *posterior collapse* (an uninformative latent code and blurry, averaged reconstructions), not literally a single repeated output — that failure mode is GAN-style mode collapse instead. These produce qualitatively different learned distributions — mode-covering versus mode-seeking.`,
    ...
    takeaway: `Minimising cross-entropy is minimising KL divergence from the true distribution to your model. Cross-entropy is not an arbitrary loss — it is the exact measure of encoding waste, and that framing tells you why the KL direction matters.`,
    recap: [

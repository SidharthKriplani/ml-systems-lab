...
const SALT = 'msl'
const STORE_KEY = 'msl-daily-drill-v1'
const EPOCH = '2026-07-16' // Drill #1
const LAB_NAME = 'ML Systems Lab'
const SHARE_URL = 'https://ml-systems-lab-v9xe.vercel.app'

async function loadPool() {
  const m = await import('../data/drills/drillPool.js')
  const labels = m.SUBJECT_LABELS || {}
  return (m.DRILL_POOL || [])
    .filter(d => d.type === 'mcq' && Array.isArray(d.options) && d.options.length >= 2 && Number.isInteger(d.answer) && d.answer >= 0 && d.answer < d.options.length)
    .map(d => ({
      qid: d.id,
      tag: labels[d.subject] || d.subject || '',
      level: d.level || '',
      context: Array.isArray(d.context) ? d.context.join(' ') : (d.context || ''),
      question: d.question || d.title || '',
      options: d.options,
      answer: d.answer,
      explanation: d.explanation || d.diagnosis || '',
    }))
}

const T = {
  text: 'var(--ink-hi)', mid: 'var(--ink-mid)', low: 'var(--ink-low)', ghost: 'var(--ink-ghost)',
  border: 'var(--rim)', surface: 'var(--surface)', bg: 'var(--depth)',
  accent: 'var(--prime)', accentFaint: 'var(--prime-faint)', accentText: 'var(--prime-hi, var(--prime))',
  green: '#34d399', red: '#e05050',
  sans: 'var(--font-sans)', mono: 'var(--font-mono, ui-monospace, monospace)',
}
  ...
  return (
    <div style={card} className={'dd-card' + (justSolved ? ' dd-solved' : '')}>
      <style>{`
        ...
        .dd-card .dd-opt:not(:disabled):hover { border-color: ${'var(--prime)'}; transform: translateX(2px); }

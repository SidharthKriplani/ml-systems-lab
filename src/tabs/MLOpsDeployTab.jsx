...
const CC_QUESTIONS = [
  {
    ...
    options: [
      { id: 'promote',    label: 'Promote challenger — AUC-PR, ECE, and false positive rate all improved' },
      { id: 'keep',       label: 'Keep champion — latency and memory regressions outweigh accuracy gains' },
      { id: 'conditions', label: 'Promote with conditions — resolve the p99 latency issue before shipping' },
      { id: 'reject',     label: 'Reject and iterate — training time increase signals over-engineering' },
    ],
    correct: 'conditions',
    reasoning: 'The p99 latency jump from 45ms → 89ms is a red flag if there\'s an SLA. Accuracy gains are real but you need to resolve the latency issue first before promoting. Option B (keep champion) would be right if the latency regression were unresolvable — but the right move is to investigate and fix, not surrender the accuracy gains. Option D treats training time as a ship gate, which it is not.',
  ...
  },
  {
    ...
    options: [
      { id: 'reject',    label: 'Reject the challenger — 3ms over SLA means it fails the promotion gate' },
      { id: 'profile',   label: 'Profile and optimize the hot path before making a ship/no-ship decision' },
      { id: 'lowtraffic', label: 'Deploy to low-traffic segments only where p99 is within SLA' },
      { id: 'instance',  label: 'Upgrade to a larger instance type to bring p99 within SLA' },
    ],
    correct: 'profile',
    reasoning: 'The accuracy gains are real and worth preserving. First: profile what\'s causing the extra 50ms (is it the new features? model size? I/O?). Often 80% of latency gain comes from one inefficiency. Don\'t throw away a better model because of an undiagnosed bottleneck. Option C (low-traffic only) is a reasonable interim step but leaves most users on the inferior champion. Option D (larger instance) is a valid operational lever but treats the symptom before diagnosing the cause — a profiling session might find a 40ms gain without any infra change.',
  ...
  },
  {
    ...
    options: [
      { id: 'immediately',  label: 'Yes — promote to 100% immediately, latency is now well within SLA' },
      { id: 'canary',       label: 'Yes — canary ramp 5%→20%→50%→100% with manual approval gates' },
      { id: 'rollback',     label: 'Yes — canary ramp with automatic rollback trigger on metric breach' },
      { id: 'keeptest',     label: 'No — run another week of shadow mode to confirm the optimization holds' },
    ],
    correct: 'rollback',
    reasoning: 'Ramp 5% → 20% → 50% → 100% with automatic rollback trigger: if p99 > 80ms OR false positive rate > 5% over 1-hour window → auto-rollback to champion. Option B (canary with manual gates) is better than immediate full deploy but worse than automatic triggers — human approvals introduce delay and overnight gaps. Automatic rollback is the production standard because metric breaches can happen at 3am.',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {activeModuleData && (
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <BookmarkButton tabId="mlops_deploy" moduleId={active} label={activeModuleData.label} />
        </div>
      )}

...
const MCQ_QUESTIONS = [
  ...
  { id: 'C41', domain: 'Feature Engineering', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Embedding drift can be caught by tracking per-category centroid movement via cosine similarity over time windows. Sudden drops signal distributional shift or data pipeline issues before model performance degrades.' },
  { id: 'C42', domain: 'Feature Engineering', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Feature hashing is O(1) lookup with no vocabulary — ideal for streaming and cold-start features. The cost is hash collisions: distinct categories map to the same bucket, injecting noise that hurts precision.' },
  { id: 'C43', domain: 'Feature Engineering', type: 'mcq',
    ...
    correct: 1,
    explanation: 'A feature store with unified feature definitions ensures the same transformation code runs offline and online, eliminating logic divergence. Separate pipelines almost always diverge over time.' },
  { id: 'C44', domain: 'Feature Engineering', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Tree models find monotonic ordinal splits naturally with a single numeric feature; one-hot forces the tree to learn the ordering implicitly using multiple binary features, wasting splits and increasing variance.' },
  { id: 'C45', domain: 'Feature Engineering', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Point-in-time correctness requires the window boundary to be anchored to the feature request time (event timestamp), not the label timestamp. Using the label timestamp includes future observations, creating look-ahead leakage.' },
  { id: 'C46', domain: 'Feature Engineering', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Equal-width bins concentrate most data in a few buckets under skewed distributions. Quantile binning guarantees each bin has the same proportion of samples, preserving information across the full distribution.' },
  { id: 'C47', domain: 'Model Evaluation', type: 'mcq',
    ...
    correct: 1,
    explanation: 'ECE partitions predictions into M bins by confidence, computes |accuracy - confidence| per bin weighted by bin size, and averages. A perfectly calibrated model has ECE = 0: predicted 80% confidence should be correct 80% of the time.' },
  { id: 'C48', domain: 'Model Evaluation', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Threshold tuning on validation data is a form of optimization that can overfit to that split. The true generalization estimate requires a held-out test set where no decision (including threshold) was made.' },
  { id: 'C49', domain: 'Model Evaluation', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Micro-F1 aggregates TP/FP/FN globally before computing F1, so large classes dominate. Macro-F1 computes per-class F1 and averages. A large gap reveals the model performs differently across class sizes.' },
  { id: 'C50', domain: 'Model Evaluation', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Logs collected under a prior ranker are not IID samples — items that were never ranked high were never clicked. The offline metric is optimistic because it only evaluates relevance on items the old ranker selected.' },
  { id: 'C51', domain: 'Model Evaluation', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Platt scaling learns two parameters (A, B) by fitting sigmoid(A·score + B) to calibration labels. It is cheap, post-hoc, and effective for SVMs and GBTs whose raw outputs are not well-calibrated probabilities.' },
  { id: 'C52', domain: 'ML Systems', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Schema-on-write enforces types, nullability, and ranges when features are written to the store, failing fast at pipeline time. Schema-on-read defers validation until serving, letting corrupt data reach inference silently.' },
  { id: 'C53', domain: 'ML Systems', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Cross-encoders jointly encode query+item (full attention), producing accurate scores but requiring inference per candidate — infeasible for millions of items. Two-tower separates encoders, precomputes item side, and uses ANN for retrieval.' },
  { id: 'C54', domain: 'ML Systems', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Exactly-once in Flink requires (1) periodic checkpoints persisting operator state and Kafka offsets atomically, and (2) Kafka transactional producer so output is committed only when the checkpoint succeeds.' },
  { id: 'C55', domain: 'ML Systems', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Redis is optimized for single-key lookups at millisecond latency but is not suited for full dataset scans during training. The offline store enables efficient bulk reads for training while the online store handles serving.' },
  { id: 'C56', domain: 'ML Systems', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Reproducibility requires the full artifact graph: weights define the function, preprocessing defines input transformation, feature schema defines expected inputs, and data version reference pins what the model learned from.' },
  { id: 'C57', domain: 'Statistics & Probability', type: 'mcq',
    ...
    correct: 1,
    explanation: 'CLT guarantees normality of sample means as n grows, but non-mean statistics (median, quantiles, AUC) have complex sampling distributions. Bootstrap resampling empirically estimates those distributions without parametric assumptions.' },
  { id: 'C58', domain: 'Statistics & Probability', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Bonferroni controls FWER at α by requiring each test to meet α/m significance. It is valid but over-conservative when tests are correlated (e.g., correlated features), inflating Type II error and discarding real signals.' },
  { id: 'C59', domain: 'Statistics & Probability', type: 'mcq',
    ...
    correct: 1,
    explanation: 'p-value is P(|T| ≥ |t_obs| | H₀). It is NOT the probability H₀ is true. Misinterpreting p-values as posterior probabilities for hypotheses is one of the most common errors in applied ML experimentation.' },
  { id: 'C60', domain: 'Statistics & Probability', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Revenue distributions are typically log-normal. log(1+x) handles zeros and compresses the right tail, reducing variance substantially. This makes the CLT approximation valid at smaller sample sizes.' },
  { id: 'C61', domain: 'Statistics & Probability', type: 'mcq',
    ...
    correct: 1,
    explanation: 'A ratio f(X,Y) = X/Y has no simple closed-form variance. Delta method approximates Var(f) using the gradient of f at the mean, enabling standard error computation for metrics like CTR = clicks/impressions.' },
  { id: 'C62', domain: 'Statistics & Probability', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Gelman & Carlin (2014): underpowered studies often produce estimates with the wrong sign (Type S) or wildly inflated/deflated magnitude (Type M). These are more decision-relevant than the binary Type I/II framing.' },
  { id: 'C63', domain: 'Deep Learning', type: 'mcq',
    ...
    correct: 1,
    explanation: 'GQA (Ainslie et al., 2023) groups G query heads to share one K and V head. KV cache shrinks by factor G relative to MHA, reducing memory bandwidth at inference — critical for long-context LLM serving.' },
  { id: 'C64', domain: 'Deep Learning', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Autoregressive decode is memory-bandwidth-bound: each step loads all cached K/V tensors. INT8 quantization halves bandwidth vs FP16, roughly doubling throughput with minimal perplexity degradation.' },
  { id: 'C65', domain: 'Deep Learning', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Gradient checkpointing (Chen et al., 2016) selects checkpoints every √N layers, storing only those activations and recomputing segments during backward pass. Memory drops from O(N) to O(√N) at ~33% extra compute cost.' },
  { id: 'C66', domain: 'Deep Learning', type: 'mcq',
    ...
    correct: 1,
    explanation: 'LoRA decomposes weight updates as ΔW = BA where B ∈ ℝ^{d×r}, A ∈ ℝ^{r×k}. Small r (1-8) limits the expressiveness of the update but drastically reduces parameters, sufficient for tasks similar to pretraining.' },
  { id: 'C67', domain: 'Deep Learning', type: 'mcq',
    ...
    correct: 1,
    explanation: 'FlashAttention (Dao et al., 2022) tiles the Q, K, V matrices to fit in SRAM (fast), computes attention in blocks, and accumulates results without writing the N×N attention matrix to HBM, reducing memory I/O from O(N²) to O(N).' },
  { id: 'C68', domain: 'Deep Learning', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Speculative decoding (Leviathan et al., 2022) parallelizes draft generation and verification. The large model verifies k tokens in one pass (same cost as generating 1 token), yielding 2–3× throughput gains with identical outputs.' },
  { id: 'C69', domain: 'MLOps', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Progressive (canary) rollout catches regressions before they affect all users. Each gate compares the canary\'s error rate, latency, and business KPIs against the baseline; a breach triggers automated rollback.' },
  ...
  { id: 'C71', domain: 'MLOps', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Artifact stores (S3/GCS) hold binary files. A model registry adds metadata: who promoted this model, what its metrics are, which experiment produced it, and what stage it is in — enabling governance and automated promotion gates.' },
  { id: 'C72', domain: 'MLOps', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Shadow mode validates the challenger\'s prediction quality and latency under real traffic with zero user impact. A/B testing requires splitting real users and affects experience, whereas shadow mode is purely observational.' },
  { id: 'C73', domain: 'MLOps', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Lion tracks only first-moment momentum and applies sign(momentum) as the update direction with a fixed LR. This removes the second-moment buffer, saving 33% memory vs Adam, while matching or exceeding Adam\'s performance on vision and language tasks.' },
  { id: 'C74', domain: 'MLOps', type: 'mcq',
    ...
    correct: 1,
    explanation: 'ML CI/CD must validate that code changes don\'t degrade model behavior: a smoke train catches pipeline breakage, a regression test catches performance drops, and schema validation catches silent input contract changes.' },
  { id: 'C75', domain: 'Ranking & Retrieval', type: 'mcq',
    ...
    correct: 1,
    explanation: 'PQ splits a d-dimensional vector into M d/M-dimensional sub-vectors, each quantized to k centroids. Storage drops from d·32 bits to M·log₂(k) bits — typically 32× compression with controllable recall degradation.' },
  { id: 'C76', domain: 'Ranking & Retrieval', type: 'mcq',
    ...
    correct: 1,
    explanation: 'HNSW\'s top layers act like a skip list: each layer is a random subset of nodes with longer edges. Greedy search descends from coarse to fine, arriving near the query\'s neighborhood with O(log N) hops before brute-force search in the base layer.' },
  { id: 'C77', domain: 'Ranking & Retrieval', type: 'mcq',
    ...
    correct: 1,
    explanation: 'LambdaRank (Burges et al.) weights the pairwise gradient by |ΔNDCG| — the change in NDCG from swapping item positions. This connects the loss to the evaluation metric without requiring a differentiable NDCG proxy.' },
  { id: 'C78', domain: 'Ranking & Retrieval', type: 'mcq',
    ...
    correct: 0,
    explanation: 'Users click higher-ranked items more often regardless of quality (position bias). Training on raw clicks encodes rank position into the model\'s relevance signal. Inverse Propensity Scoring (IPS) or unbiased LTR methods debias click labels.' },
  { id: 'C79', domain: 'Ranking & Retrieval', type: 'mcq',
    ...
    correct: 1,
    explanation: 'MMR selects items by arg max λ·sim(item, query) − (1−λ)·max_{s∈Selected} sim(item, s). Parameter λ trades off relevance vs. diversity, avoiding redundant results and improving coverage of user information needs.' },
  { id: 'C80', domain: 'Ranking & Retrieval', type: 'mcq',
    ...
    correct: 1,
    explanation: 'ColBERT produces per-token embeddings for query and document. Relevance = Σ_{q_i} max_{d_j} q_i·d_j (MaxSim). This is richer than bi-encoder single-vector dot product and cheaper than full cross-encoder attention at query time.' },
  { id: 'C81', domain: 'Experiment Design', type: 'mcq',
    ...
    correct: 1,
    explanation: 'CUPED computes Y_cuped = Y − θ·(X_pre − E[X_pre]) where X_pre is a pre-experiment covariate correlated with Y. This covariate adjustment removes variance explained by baseline behavior, often reducing variance by 50–70%.' },
  { id: 'C82', domain: 'Experiment Design', type: 'mcq',
    ...
    correct: 1,
    explanation: 'If Test A changes the recommendation algorithm and Test B changes the UI, users assigned to A1+B1 may show non-additive effects. Mutual exclusion (separate user pools) or factorial design (all combinations) are the standard mitigations.' },
  { id: 'C83', domain: 'Experiment Design', type: 'mcq',
    ...
    correct: 1,
    explanation: 'SUTVA (Stable Unit Treatment Value Assumption) requires no interference between units. Social platforms violate this because treated users affect controls. Cluster randomization (by friend graph, city, or device) isolates spillover within clusters.' },
  { id: 'C84', domain: 'Experiment Design', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Users engage more with any novel change initially. If the experiment is too short, the inflated novelty effect will be measured instead of the steady-state effect, leading to incorrectly shipping a feature that provides no lasting value.' },
  { id: 'C85', domain: 'Experiment Design', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Fixed-horizon tests are invalidated by peeking. Alpha spending functions distribute the significance budget (e.g., 0.005, 0.01, 0.025, 0.05 across 4 looks) so the total false positive rate stays at α = 0.05 across all analyses.' },
  { id: 'C86', domain: 'Experiment Design', type: 'mcq',
    ...
    correct: 1,
    explanation: 'A holdback group withheld from all new features accumulates the baseline. Comparing production vs. holdback after 6–12 months gives an unbiased estimate of total product improvement — catching cases where many small positive tests combine to have no net effect.' },
  { id: 'C87', domain: 'SQL & Data', type: 'mcq',
    ...
    correct: 0,
    explanation: 'Window functions require rows sorted by PARTITION BY + ORDER BY. A covering index on (user_id, ts) eliminates the sort step and allows the engine to stream rows in the required order, converting an O(N log N) sort to O(N) scan.' },
  { id: 'C88', domain: 'SQL & Data', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Most SQL engines (BigQuery, Postgres) do not automatically memoize CTEs referenced multiple times — they re-execute the CTE per reference. A temp table forces materialization, saving repeated computation at the cost of I/O.' },
  { id: 'C89', domain: 'SQL & Data', type: 'mcq',
    ...
    correct: 1,
    explanation: 'A Bloom filter for a column\'s row group encodes which values are present. A query predicate (WHERE id = X) checks the filter — false means skip the row group entirely. False positives cause unnecessary reads but no incorrect results.' },
  { id: 'C90', domain: 'SQL & Data', type: 'mcq',
    ...
    correct: 1,
    explanation: 'SCD Type 2 preserves history by inserting new rows on change. effective_start_date and effective_end_date (NULL or sentinel for active records) allow point-in-time queries: WHERE event_date BETWEEN effective_start_date AND COALESCE(effective_end_date, \'9999-01-01\').' },
  { id: 'C91', domain: 'SQL & Data', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Z-ordering maps multi-dimensional column values to a 1D Z-curve, placing rows with similar combinations of (col_A, col_B) values in the same files. Queries filtering on either column or both benefit from file skipping via Delta\'s min/max statistics.' },
  { id: 'C92', domain: 'SQL & Data', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Hash join builds a hash table on the smaller (build) side and probes it with the larger (probe) side — O(N+M). Sort-merge requires both sides sorted — O(N log N + M log M) if unsorted. Hash join dominates when the build side fits in memory.' },
  { id: 'C93', domain: 'ML Systems', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Cost-aware routing reserves GPU capacity for high-complexity inputs where latency matters most. A lightweight complexity estimator (e.g., input length threshold) can reduce GPU spend 40–60% with negligible latency impact for simple requests.' },
  { id: 'C94', domain: 'Optimization', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Gradient accumulation sums gradients over k micro-batches before calling optimizer.step(), achieving effective batch size k × micro_batch_size. This allows large-batch training on memory-constrained GPUs without communication overhead of increased batch size.' },
  { id: 'C95', domain: 'Optimization', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Inspired by human learning, curriculum learning (Bengio et al., 2009) improves convergence speed and generalization by presenting easy examples first. In NLP, this can mean shorter sequences or high-frequency token sequences before complex ones.' },
  { id: 'C96', domain: 'Optimization', type: 'mcq',
    ...
    correct: 1,
    explanation: 'SWA (Izmailov et al., 2018) periodically snapshots weights and maintains a running average. The averaged weights occupy a wider, flatter basin than individual SGD solutions, improving generalization without training extra models.' },
  { id: 'C97', domain: 'Optimization', type: 'mcq',
    ...
    correct: 1,
    explanation: 'Global norm clipping: if ‖g‖₂ > clip_value, g ← g · (clip_value / ‖g‖₂). This preserves gradient direction (unlike per-parameter clipping which distorts it) while bounding step size, critical for RNN and transformer training stability.' },
  { id: 'C98', domain: 'Optimization', type: 'mcq',
    ...
    correct: 1,
    explanation: 'FP16 dynamic range is 5.96×10⁻⁸ to 65504. Small gradients underflow to 0, causing training to stall. Loss scaling multiplies loss by a scale factor S before backward, gradients become S× larger, staying above the underflow threshold, then unscaled before optimizer.' },
  { id: 'C99', domain: 'Optimization', type: 'mcq',
    ...
    correct: 1,
    explanation: 'ZeRO Stage 1 shards optimizer states, Stage 2 adds gradient sharding, Stage 3 adds parameter sharding. Stage 3 reduces per-GPU memory to O(total_params/N) at the cost of all-gather communication during forward/backward passes.' },
  { id: 'C100', domain: 'Model Evaluation', type: 'mcq',
    ...
    correct: 1,
    explanation: 'McNemar\'s test uses the 2×2 contingency table of (correct/wrong) pairs across classifiers. It tests H₀: both models have the same error rate using only the discordant pairs, giving a more powerful paired test than treating predictions as independent samples.' },
]

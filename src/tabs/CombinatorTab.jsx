...
const MCQ_QUESTIONS = [
  ...
  { id: 'C3', domain: 'Feature Engineering', type: 'mcq',
    ...
    correct: 1,
    explanation: "Fit imputer on train, transform both train and test. Using full dataset leaks test statistics into training. Production tell: validation AUC is suspiciously close to train AUC; model degrades sharply on truly held-out data collected after the training cutoff." },
  ...
  { id: 'C6', domain: 'Model Evaluation', type: 'mcq',
    ...
    correct: 2,
    explanation: "NDCG@K captures position-weighted relevance. For ranking, position matters — relevant item at rank 1 is more valuable than at rank 10. Production tell: AUC looks flat but user engagement drops; NDCG@5 reveals the model is burying relevant items below rank 5 where users rarely scroll." },
  ...
  { id: 'C8', domain: 'Model Evaluation', type: 'mcq',
    ...
    correct: 1,
    explanation: "Pearson measures linear correlation. A model can have high Pearson but poor calibration (all predictions scaled wrong). Use Spearman for monotone, check calibration curves separately. Production tell: downstream system uses predicted probabilities as scores for a threshold rule; miscalibration causes 40% of positives to be missed despite high AUC." },
  // ML Systems
  { id: 'C9', domain: 'ML Systems', type: 'mcq',
    ...
    correct: 1,
    explanation: "Streaming systems (Flink/Spark SS) use watermarks to bound lateness. Events within watermark window are processed; beyond it trigger late-data handling (side output or drop). In production this breaks as: watermark set too tight drops 15% of mobile events from flaky connections; aggregates are systematically undercounted and fraud model misses spikes." },
  ...
  { id: 'C15', domain: 'Statistics & Probability', type: 'mcq',
    ...
    correct: 2,
    explanation: "Bayesian: compute P(B>A | data) continuously. No multiple comparisons problem for peeking. Frequentist p-values are invalid if you peek and stop early — optional stopping inflates α. In production this breaks as: analyst checks p-value daily and stops at first p<0.05; actual false positive rate is closer to 30%, not 5%." },
  ...
  { id: 'C18', domain: 'Deep Learning', type: 'mcq',
    ...
    correct: 2,
    explanation: "Lower layers encode general features (syntax, basic semantics). Upper layers encode task-specific features. Fine-tune top layers first, optionally unfreeze lower layers with smaller LR. In production this breaks as: fine-tuning all layers with a single high LR catastrophically forgets general representations; validation loss explodes after epoch 1 on a small domain dataset." },
  { id: 'C19', domain: 'Deep Learning', type: 'mcq',
    ...
    correct: 2,
    explanation: "Large batches → sharper minima → worse generalization (Keskar et al.). Linear scaling rule: when doubling batch size, double LR and use warmup to partially compensate. Production tell: migrating from 8-GPU to 64-GPU training with naive batch scaling; validation accuracy drops 2-3 points even though training loss converges similarly." },
  ...
  { id: 'C23', domain: 'MLOps', type: 'mcq',
    ...
    correct: 1,
    explanation: "App monitoring: latency, error rate, uptime. Model monitoring additionally requires: feature drift (PSI), prediction drift, label feedback quality, calibration, and business metric correlation. Production tell: infra dashboards all green but CTR drops 20%; only model monitoring on prediction score distribution would have caught the drift three days earlier." },

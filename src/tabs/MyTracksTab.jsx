...
const DIFF_COLORS = {
  foundational: { color: 'var(--ink-low)',  bg: 'transparent', border: 'var(--rim)' },
  intermediate: { color: 'var(--prime)',    bg: 'transparent', border: 'var(--prime)' },
  advanced:     { color: '#e05050',         bg: 'transparent', border: '#e05050' },
}
    ...
    <div style={{
      ...
      {tracks.length === 0 && !creating && (
        <div style={{ padding: '0.5rem 0.75rem', color: 'var(--ink-low)', fontSize: '0.8rem', lineHeight: 1.5 }}>
          No tracks yet. Hit + to create one, then add modules with the + button inside any foundation.
        </div>
      )}
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem 2rem', minWidth: 0 }}>
      ...
      <div style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
        ...
        <span style={{ marginLeft: 'auto', fontSize: '0.75rem', color: 'var(--ink-ghost)' }}>
      ...
      {track.items.length === 0 ? (
        <div style={{ color: 'var(--ink-low)', fontSize: '0.85rem', lineHeight: 1.6, padding: '1rem 0' }}>
          This track is empty. Open any foundation tab and click the + button in a module to add it here.
        </div>
      ) : (
          ...
          {track.items.map((item, idx) => (
            <div
              ...
              style={{
                ...
                padding: '0.7rem 0.85rem', borderRadius: '8px', marginBottom: '0.35rem',
            ...
            >
              {/* Drag handle */}
              <span style={{ color: 'var(--ink-ghost)', fontSize: '0.7rem', marginTop: '2px', userSelect: 'none', flexShrink: 0 }}>⠿</span>
              ...
              {item.type === 'module' ? (
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '0.68rem', fontWeight: 700, color: 'var(--ink-low)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.2rem' }}>Note</div>
                  <div style={{ fontSize: '0.83rem', color: 'var(--ink-mid)', lineHeight: 1.5 }}>{item.content}</div>
                </div>
              )}
              ...
              <button
                ...
                title="Remove"
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink-ghost)', fontSize: '0.72rem', padding: '0.1rem 0.2rem', flexShrink: 0, marginTop: '1px' }}
      ...
      {/* Add note */}
      <form onSubmit={handleAddNote} style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
        <textarea
          value={noteText}
          ...
          rows={2}
          style={{
            flex: 1, fontSize: '0.83rem', padding: '0.5rem 0.7rem',
            background: 'var(--surface)', border: '1px solid var(--rim)',
            borderRadius: '7px', color: 'var(--ink-hi)', outline: 'none',
        ...
        <button
          ...
          style={{
            background: 'var(--prime)', color: '#fff', border: 'none', borderRadius: '7px',
            padding: '0.5rem 0.85rem', cursor: 'pointer', fontSize: '0.8rem',
            fontWeight: 600, opacity: noteText.trim() ? 1 : 0.4, flexShrink: 0,
          }}
        >Add Note</button>
      </form>
    </div>
...
export function MyTracksTab({ onNavigate }) {
  ...
  // Refresh when tracks change from other components
  useEffect(() => {
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 48px)', fontFamily: 'var(--font-sans)', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', background: 'var(--depth)' }}>
        {selectedTrack ? (
          <TrackDetail

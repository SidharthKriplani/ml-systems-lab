...
//   • Cross-domain incidents -> multi-step production incidents (IncidentRoomTab)
//   • Company cases          -> company scenarios, MCQ + open (CaseStudiesTab)
...
export default function CaseRoomTab({ onNavigate, openModuleId }) {
  ...
  const pick = (m) => { setMode(m); try { localStorage.setItem('msl_caseroom_mode', m) } catch {} }
  const seg = (active) => ({
    ...
    <div>
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '8px 4px 0' }}>
        <div style={{ display: 'flex', border: '1px solid var(--rim)', borderRadius: 8, overflow: 'hidden', background: 'var(--surface)', marginBottom: 4 }}>
          <button style={seg(mode === 'incident')} onClick={() => pick('incident')}>Cross-domain incidents</button>
          <button style={seg(mode === 'company')} onClick={() => pick('company')}>Company cases</button>
        </div>

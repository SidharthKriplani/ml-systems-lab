...
function MarkDoneButton({ moduleId, onDone, allAnswered }) {
  ...
  return done ? (
    ...
    <button onClick={() => { if (allAnswered) { markModuleDone(moduleId); onDone() } }}
      ...
      {allAnswered ? 'Mark as reviewed' : 'Attempt all check questions'}
    </button>
  )
}

// Rendered in place of authored content for modules that are still specced-only.
function SkeletonBody({ module }) {
  return (
    <>
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.5rem' }}>
          <span style={{ fontSize: '0.68rem', fontWeight: 700, color: '#b45309',
            textTransform: 'uppercase', letterSpacing: '0.08em' }}>In development</span>
        </div>
        <p style={{ fontSize: '0.9rem', color: 'var(--ink-mid)', lineHeight: 1.6, margin: 0 }}>
          This module is <strong style={{ color: 'var(--ink-hi)' }}>specced but not yet authored</strong>.
          The full teaching pass — worked numerical examples, key insights, an interactive, and check
          questions — is on the way. The outline below is the real blueprint for what it will cover.
        </p>
      </div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--rim)', borderRadius: '10px',
        ...
        <div style={{ fontSize: '0.68rem', fontWeight: 700, color: 'var(--prime)', textTransform: 'uppercase',
          letterSpacing: '0.08em', marginBottom: '0.6rem' }}>What this module will cover</div>
        {renderMd(module.spec || module.subtitle, { fontSize: '0.9rem', color: 'var(--ink-mid)', lineHeight: 1.7, margin: 0 })}
      </div>

      <div style={{ background: 'var(--surface)', border: '1px solid var(--rim)', borderRadius: '10px',
        padding: '1.1rem 1.25rem', marginBottom: '1.5rem' }}>
        <div style={{ fontSize: '0.68rem', fontWeight: 700, color: 'var(--prime)', textTransform: 'uppercase',
          letterSpacing: '0.08em', marginBottom: '0.6rem' }}>Planned components</div>
        {['Concept walkthrough with a worked numerical example',
          'Key insight + key points',
          'Interactive visualization',
          'Check questions + quick recap'].map((c, i) => (
          <div key={i} style={{ display: 'flex', gap: '0.6rem', marginBottom: '0.5rem', alignItems: 'flex-start' }}>
            <span style={{ color: 'var(--ink-low)', fontWeight: 700, flexShrink: 0, fontSize: '0.85rem', lineHeight: 1.55 }}>▹</span>
            <div style={{ fontSize: '0.86rem', color: 'var(--ink-low)', lineHeight: 1.55 }}>{c}</div>
          </div>
        ))}
      </div>
    </>
...
export function PricingFoundationTab({ onNavigate, openModuleId, navOrigin }) {
  ...
  // Close track popover + exit recap mode when module selection changes
  ...
  const isSkeleton = selected && selected.skeleton
        ...
        <div style={{ marginBottom: '1.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', flexWrap: 'wrap' }}>
            <h2 style={{ fontSize: '1.3rem', fontWeight: 800, color: 'var(--ink-hi)', margin: 0, letterSpacing: '-0.02em' }}>
              Pricing Analytics Foundations
            </h2>
            <span style={{ fontSize: '0.62rem', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase',
              color: 'var(--amber, #d97706)', border: '1px solid var(--amber, #d97706)', borderRadius: '5px', padding: '0.1rem 0.4rem' }}>
              In development
            </span>
          </div>
          <p style={{ fontSize: '0.8rem', color: 'var(--ink-low)', margin: '0.4rem 0 0', lineHeight: 1.4 }}>
            Price elasticity, dynamic/surge pricing, price optimization under constraints, causal price experiments (A/B + geo + switchback), promotion uplift, willingness-to-pay, and revenue-vs-margin objective design. This premium niche is specced now; modules are being authored.
          </p>
            ...
            >
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: '0.72rem', color: 'var(--ink-low)', marginTop: '0.15rem' }}>
                  {m.estimatedMin} min · {m.difficulty}{m.skeleton ? ' · spec' : ''}
                </div>
              </div>
            </div>
          )

...
function ModelSelectionOracle() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div>
        <h3 style={{ fontFamily: 'var(--font-sans)', fontSize: '18px', fontWeight: 800, color: 'var(--prime)', letterSpacing: '-0.02em', marginBottom: '4px' }}>Model Selection Oracle</h3>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '16px' }}>
        {[
          { label: 'Task type', items: TASK_TYPES, val: taskType, set: setTaskType, color: 'var(--prime)' },
          { label: 'Data size', items: DATA_SIZES, val: dataSize, set: setDataSize, color: 'var(--prime)' },
          { label: 'Interpretability requirement', items: INTERP, val: interp, set: setInterp, color: 'var(--prime)' },
          { label: 'Feature type', items: FEAT_TYPES, val: featType, set: setFeatType, color: 'var(--prime)' },
        ].map(g => (
              ...
              {expanded === m.id && (
                <div style={{ padding: '0 18px 18px', borderTop: '1px solid var(--rim)', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', paddingTop: '14px' }}>
                    <div>
                      <div style={{ fontSize: '10px', color: 'var(--prime)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '6px' }}>Strengths</div>
                      {m.strengths.map((s, j) => <div key={j} style={{ fontSize: '12px', color: 'var(--ink-mid)', lineHeight: 1.6, marginBottom: '3px' }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{display:"inline-block",verticalAlign:"text-bottom",marginRight:"3px"}}><polyline points="20 6 9 17 4 12"/></svg>{s}</div>)}
                    </div>
                    <div>
                      <div style={{ fontSize: '10px', color: 'var(--ink-low)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '6px' }}>Watch out for</div>
                      {m.risks.map((r, j) => <div key={j} style={{ fontSize: '12px', color: 'var(--ink-mid)', lineHeight: 1.6, marginBottom: '3px' }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{display:"inline-block",verticalAlign:"text-bottom",marginRight:"3px"}}><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3.05h16.94a2 2 0 0 0 1.71-3.05L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>{r}</div>)}
                    </div>
                  </div>
...
const STATS_SCENARIOS = [
  {
    ...
    options: [
      ...
      'They should have used a sequential testing method but ran a fixed-horizon test instead',
    ],
    correct: 0,
    mistake: 'Peeking / multiple comparisons',
    explanation: 'Checking results repeatedly and stopping when significant inflates the Type I error rate. If you check 5 times at α=0.05, actual false positive rate ≈ 23%. Each look is an independent opportunity to get lucky. Option D identifies a real method but misframes the problem — the mistake is peeking at a fixed-horizon test multiple times, not simply failing to use sequential methods from the start.',
  ...
  },
  {
    ...
    options: [
      'Each test needs a separate holdout group to avoid user assignment conflicts',
      'Multiple comparisons without correction — inflated family-wise error rate',
      'Tests should share a unified control group to reduce required sample size',
      'The experiments should be run sequentially to prevent novelty effect contamination',
    ],
    correct: 1,
    mistake: 'Multiple comparisons without correction',
    explanation: 'Running 10 independent tests at α=0.05 means a ~40% chance of at least one false positive (1 − 0.95^10 ≈ 0.40). You will ship a broken feature. Option A (separate holdout) is a real concern in multi-test platforms but is orthogonal to the primary failure here. Option C (shared control) is a legitimate efficiency technique in multi-armed experiments but does not address the false discovery rate problem.',
  ...
  },
  {
    ...
    options: [
      'Post-hoc power analysis confirmed the test was underpowered — collecting more data is the correct remedy',
      'p-hacking / data dredging — inflates false positive rate to whatever you are willing to wait for',
      'The MDE was too small for the original sample size — the fix is to run a new pre-registered experiment with adequate power',
      'Should have used a one-sided test to gain additional sensitivity',
    ],
    correct: 1,
    mistake: 'p-hacking / data dredging',
    explanation: 'Running until significant is the definition of p-hacking. You are exploiting random variation in cumulative data. Inflate your sample long enough and almost any noise will eventually cross 0.05. Option A is the trap most practitioners fall into — post-hoc power analysis on a completed experiment is misleading (it will always show the test was underpowered when p > 0.05). Option C describes the correct next step IF you believe the effect is real and worth testing — but it requires starting fresh with a pre-registered design, not extending the existing run.',
    fix: 'Pre-register sample size via power analysis before the experiment. If the first run did not reach significance, the lift may not exist or not be practically meaningful. If you believe the effect is real, design a new pre-registered experiment with sufficient power.',
  },
  {
    id: 7,
  ...
  },
  {
    ...
    options: [
      ...
      'Should apply CUPED variance reduction before running the t-test',
      'Violated test assumptions — t-test is unreliable on heavily skewed, heavy-tailed distributions',
      'Should use a log-transform on revenue, then run the t-test on the transformed values',
    ],
    correct: 2,
    mistake: 'Violated test assumptions',
    explanation: 'The t-test assumes approximately normal sampling distribution of the mean. With a heavy-tailed revenue distribution (a few whales driving most revenue), the Central Limit Theorem convergence is very slow. The resulting p-values are unreliable. Option B (CUPED) reduces variance by removing pre-experiment covariate noise — valid but doesn\'t address the skewness assumption. Option D (log-transform + t-test) is actually a legitimate mitigation, but it changes the estimand: you\'re now measuring relative lift on log-revenue, not absolute lift on revenue, which is a different business question.',
    fix: 'Use Mann-Whitney U test (non-parametric), bootstrap the difference in means (10k iterations), or log-transform revenue per user before applying the t-test — but document that the latter changes the estimand to relative lift.',
  },
]
...
function StatisticalTestingPitfalls() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div>
        <h3 style={{ fontFamily: 'var(--font-sans)', fontSize: '18px', fontWeight: 800, color: 'var(--prime)', letterSpacing: '-0.02em', marginBottom: '4px' }}>Analysis Mistakes</h3>
        <p style={{ fontSize: '13px', color: 'var(--ink-low)', lineHeight: 1.6, margin: 0 }}>
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          {STATS_SCENARIOS.map((_, i) => (
            <div key={i} onClick={() => { setCurrent(i); setSelected(null); setRevealed(false) }}
              style={{ width: '24px', height: '6px', borderRadius: '3px', cursor: 'pointer', background: answered.includes(i) ? 'var(--prime)' : i === current ? 'rgba(240,165,0,0.4)' : 'var(--rim)', transition: 'background 0.2s' }} />
          ))}
        </div>
      ...
      {/* Scenario card */}
      <div className="card animate-slide-up" style={{ padding: '22px', borderLeft: '3px solid var(--prime)' }}>
        <div style={{ fontSize: '10px', color: 'var(--prime)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '10px' }}>Scenario {current + 1} of {STATS_SCENARIOS.length}</div>
          ...
          {s.options.map((opt, idx) => {
            ...
            return (
              <button key={idx} onClick={() => handleSelect(idx)}
                ...
                {opt}
                {revealed && idx === s.correct && <span style={{ marginLeft: 'auto', fontSize: '11px' }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{display:"inline-block",verticalAlign:"text-bottom",marginRight:"3px"}}><polyline points="20 6 9 17 4 12"/></svg></span>}
                {revealed && selected === idx && idx !== s.correct && <span style={{ marginLeft: 'auto', fontSize: '11px' }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{display:"inline-block",verticalAlign:"text-bottom",marginRight:"3px"}}><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></span>}
              </button>
            )
          })}
        </div>
      ...
      {allDone && revealed && current === STATS_SCENARIOS.length - 1 && (
        <div className="card animate-slide-up" style={{ padding: '20px', textAlign: 'center', borderLeft: '3px solid var(--prime)' }}>
...
function CalibrationInPractice() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div>
        <h3 style={{ fontFamily: 'var(--font-sans)', fontSize: '18px', fontWeight: 800, color: 'var(--prime)', letterSpacing: '-0.02em', marginBottom: '4px' }}>Calibration in Practice</h3>
        <p style={{ fontSize: '13px', color: 'var(--ink-low)', lineHeight: 1.6, margin: 0 }}>
          ...
          <div style={{ marginTop: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {CALIB_REFERENCE.map(r => (
              <div key={r.term} style={{ display: 'flex', gap: '10px' }}>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--ink-low)', minWidth: '140px', paddingTop: '1px' }}>{r.term}</span>
      ...
      {/* Scenario */}
      <div className="card animate-slide-up" style={{ padding: '22px', borderLeft: '3px solid var(--prime)' }}>
        <div style={{ fontSize: '10px', color: 'var(--prime)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '10px' }}>Scenario {current + 1} of {CALIB_SCENARIOS.length}</div>
        <p style={{ fontSize: '14px', color: 'var(--ink-hi)', lineHeight: 1.75, margin: 0, fontFamily: 'var(--font-sans)' }}>{s.scenario}</p>
      </div>
...
function MetricDesign() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div>
        <h3 style={{ fontFamily: 'var(--font-sans)', fontSize: '18px', fontWeight: 800, color: 'var(--prime)', letterSpacing: '-0.02em', marginBottom: '4px' }}>Metric Design Pitfalls</h3>
        <p style={{ fontSize: '13px', color: 'var(--ink-low)', lineHeight: 1.6, margin: 0 }}>
...
export default function DataScienceTab({ onNavigate }) {
  ...
  const activeModuleData = DS_MODULES.find(m => m.id === active)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
      ...
      {activeModuleData && (
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <BookmarkButton tabId="ds" moduleId={active} label={activeModuleData.label} />
        </div>
      )}
      ...
      {onNavigate && <ForwardPointer label="Apply causal thinking in Causal Inference" tab="causal" onNavigate={onNavigate} accent="var(--prime)" />}
    </div>
  )
}

...
function ensureStyles() {
  ...
  s.id = 'msl-hl-native-styles'
  s.textContent = Object.keys(FAINT)
    .map(function (k) { return '::highlight(msl-hl-' + k + ') { background-color: ' + FAINT[k] + '; }' })
    .join('\n')
...
function rebuildRegistry() {
  ...
  for (const name of Object.keys(FAINT)) {
    ...
    if (ranges && ranges.length) CSS.highlights.set('msl-hl-' + name, new Highlight(...ranges))
    else CSS.highlights.delete('msl-hl-' + name)
  }
}
